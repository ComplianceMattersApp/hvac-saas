...
export async function countAttentionCenterItems(params: { supabase: any; accountOwnerUserId: string }) {
  ...
  const [qboFailed, qboStripeUnsent, invoiceErrors, voidDrift, staleStripe, fieldReports, failedAttempts, qboConnection] = await Promise.all([
  ...
  return countOf(qboFailed) + countOf(qboStripeUnsent) + countOf(invoiceErrors) + countOf(voidDrift) + countOf(staleStripe)
