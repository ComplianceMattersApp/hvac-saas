...
const repair = readFileSync(resolve(__dirname, "../stripe-successful-payment-reconciliation.ts"), "utf8");
...
describe("Stripe pending-payment repair safety wiring", () => {
  ...
  it("re-fetches Stripe truth and refuses ambiguous paid sessions", () => {
    ...
    expect(repair).toContain('"ambiguous_multiple_successes"');
    expect(repair).toContain('"not_paid"');
  });

  it("requires exact scope, amount, connected account, and original Stripe event", () => {
    expect(repair).toContain("invoice_scope_mismatch");
    ...
    expect(repair).toContain("if (!event?.id)");
  });

  it("reuses webhook settlement before downstream QBO and receipt follow-through", () => {
    expect(repair).toContain("recordTenantInvoicePaymentFromCheckoutSession({ session, eventId: event.id");
  ...
  it("does not expose bulk repair", () => {
    ...
    expect(page).toContain("Reconcile confirmed payment");
  });
