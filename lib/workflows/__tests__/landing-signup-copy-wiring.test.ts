...
describe("landing and signup copy wiring", () => {
  ...
  it("renders guided trial login copy with compact preview block", () => {
    ...
    expect(loginPage).toContain("Both paths start with a 30-day guided setup and no payment details.");
  ...
  it("renders trial-focused product choice copy with compact Service and ECC preview tiles", () => {
    ...
    expect(productChoicePage).toContain("Start a 30-day guided trial");
    ...
    expect(productChoicePage).not.toContain("14-day");
    expect(productChoicePage).not.toContain("14 day");
