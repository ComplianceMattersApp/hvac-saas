...
describe("company profile success guide wiring", () => {
  it("renders Success Guide copy and plain-English checklist", () => {
    const pageSource = readWorkspaceFile("app/ops/admin/company-profile/page.tsx");

    expect(pageSource).toContain("Success Guide");
    expect(pageSource).toContain("Start with one real job. You can come back to the rest later.");
