...
import {
  CreateJobInput,
  JobAssignment,
  JobAssignmentCreatedCallback,
  OperationalEmailDeliveryStatus,
  OperationalEmailNotificationType,
  OperationalEmailRecipientType,
  SERVICE_CASE_KINDS,
  SERVICE_VISIT_OUTCOMES,
  SERVICE_VISIT_TYPES,
  addJobAssignment,
  applyJobScheduleUpdate,
  buildInitialProblemSummary,
  buildOperationalEmailDedupeKey,
  buildScheduleSignature,
  cleanupOrphanSystem,
  createJob,
  createServiceCaseForRootJob,
  deriveInitialServiceVisitReason,
  ensureActiveAssignmentAndNotify,
  ensureActiveAssignmentForUser,
  ensureServiceCaseForJob,
  findExistingOperationalEmailDelivery,
  formatServiceAddress,
  getSafeErrorDetails,
  hasOperationalEmailHistory,
  insertJobEvent,
  insertOperationalEmailDeliveryNotification,
  markOperationalEmailDeliveryNotification,
  normalizeJobTab,
  normalizeScheduleValue,
  normalizeServiceCaseKind,
  normalizeServiceVisitOutcome,
  ...
  resolveServiceCaseIdForNewJob,
  resolveSystemIdForRun,
  revalidateEccProjectionConsumers,
  sendContractorScheduledEmailForJob,
  sendCustomerScheduledEmailForJob,
  toTitleCase,
  updateJob,
...
export async function updateJobScheduleFromForm(formData: FormData) {
  ...
  let scheduleNotificationFailed = false;

  async function runScheduleNotificationSafely(step: string, work: () => Promise<void>) {
    try {
      await work();
    } catch (error) {
      scheduleNotificationFailed = true;
      console.error("[jobs] Schedule notification side-effect failed", {
        step,
        jobId: id,
        eventType: event_type,
        error: error instanceof Error ? error.message : "Unknown schedule notification error",
      });
    }
  }
  ...
  if (scheduleNotificationFailed) {
    return finishScheduleTarget("schedule_saved_notification_failed");
  }
