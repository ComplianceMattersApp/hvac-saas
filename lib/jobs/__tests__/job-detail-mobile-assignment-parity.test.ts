...
describe("mobile job detail assignment parity", () => {
  ...
  it("reuses current mobile permit, billing, external billing, and closeout blockers", () => {
    ...
    expect(mobileJobDetailV2PreviewSource).toContain('name="scheduled_date" value={displayDateLA(job.scheduled_date) ?? ""}');
    expect(mobileJobDetailV2PreviewSource).toContain('name="window_start" value={timeToTimeInput(job.window_start) ?? ""}');
    expect(mobileJobDetailV2PreviewSource).toContain('name="window_end" value={timeToTimeInput(job.window_end) ?? ""}');
    expect(mobileJobDetailV2PreviewSource).toContain('name="permit_number"');
    expect(mobileJobDetailV2PreviewSource).toContain('name="jurisdiction"');
    expect(mobileJobDetailV2PreviewSource).toContain('name="permit_date"');
    expect(mobileJobDetailV2PreviewSource).toContain("Save Permit Info");
    expect(mobileJobDetailV2PreviewSource).not.toContain('standardJobAnchorHref("mobile-permit-info")');
    ...
    expect(mobileJobDetailV2PreviewSource).toContain('"Prepare the ECC invoice now. Issue and send become available when the job is ready for closeout."');
    expect(mobileJobDetailV2PreviewSource).toContain('actionLabel: props.internalInvoiceTruth ? props.jobPageInvoiceNextAction || "Review invoice" : "Create Invoice"');
  ...
  it("keeps V2 More Details tools flattened into direct grouped rows", () => {
    ...
    expect(mobileJobDetailV2PreviewSource).toContain('id="mobile-tools"');
    expect(mobileJobDetailV2PreviewSource).toContain("Set or clear waiting and hold blockers");
    expect(mobileJobDetailV2PreviewSource).toContain("group-open/status-tools:rotate-90");
    expect(mobileJobDetailV2PreviewSource).toContain("const v2StatusToolsReturnTo = `/jobs/${job.id}?tab=${tab}&mobileLayout=v2#mobile-tools`;");
    expect(mobileJobDetailV2PreviewSource).toContain("form action={updateJobOpsFromForm}");
    expect(mobileJobDetailV2PreviewSource).toContain('name="return_to" value={v2StatusToolsReturnTo}');
    expect(mobileJobDetailV2PreviewSource).toContain("InterruptStateFields");
    expect(mobileJobDetailV2PreviewSource).toContain("initialInterruptState={currentInterruptState");
    expect(mobileJobDetailV2PreviewSource).toContain("initialStatusReason={initialInterruptReason}");
    expect(mobileJobDetailV2PreviewSource).toContain("Save Interrupt State");
    expect(mobileJobDetailV2PreviewSource).toContain("canShowReleaseAndReevaluate");
    expect(mobileJobDetailV2PreviewSource).toContain("form action={releaseAndReevaluateFromForm}");
    expect(mobileJobDetailV2PreviewSource).toContain("{interruptReleaseActionLabel}");
    expect(mobileJobDetailV2PreviewSource).not.toContain('standardJobAnchorHref("mobile-tools")');
