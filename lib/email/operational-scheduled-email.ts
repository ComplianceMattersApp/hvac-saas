...
export function buildCustomerScheduledEmailHtml(args: {
  ...
  const variant = args.variant === "updated" ? "updated" : "scheduled";
  const headerLabel = variant === "updated" ? "Appointment Updated" : "Appointment Scheduled";
  const heading = variant === "updated" ? "Your schedule has been updated" : "Your appointment is scheduled";
  const intro =
    variant === "updated"
      ? `Hi ${escapeHtml(args.customerName)}, your appointment has been rescheduled.`
      : `Hi ${escapeHtml(args.customerName)}, your service appointment has been scheduled.`;
  const summaryLabel = variant === "updated" ? "Updated Appointment Details" : "Appointment Summary";
            ...
            <table role="presentation" width="640" cellspacing="0" cellpadding="0" style="width: 100%; max-width: 640px; border-collapse: collapse; border: 1px solid #dbe4f0; border-radius: 16px; overflow: hidden; background: #ffffff; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.08);">
              <tr>
                <td style="padding: 18px 20px 10px 20px; border-bottom: 1px solid #e2e8f0; background: linear-gradient(180deg, #f8fbff 0%, #ffffff 100%);">
                  <div style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #1d4ed8; font-weight: 700; margin: 0 0 8px 0;">${escapeHtml(headerLabel)}</div>
                  ${safeLogoUrl
              ...
              <tr>
                <td style="padding: 18px 20px 0 20px;">
                  <h1 style="margin: 0; font-size: 24px; line-height: 1.25; color: #0f172a;">${escapeHtml(heading)}</h1>
                  <p style="margin: 10px 0 0 0; font-size: 15px; line-height: 1.6; color: #334155;">${intro}</p>
                </td>
              </tr>
              <tr>
                <td style="padding: 16px 20px 0 20px;">
                  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; border: 1px solid #dbe4f0; border-radius: 12px; overflow: hidden; background: #f8fbff;">
                    <tr>
                      <td colspan="2" style="padding: 10px 12px; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: #334155; font-weight: 700; border-bottom: 1px solid #dbe4f0;">${escapeHtml(summaryLabel)}</td>
                    </tr>
                    <tr>
