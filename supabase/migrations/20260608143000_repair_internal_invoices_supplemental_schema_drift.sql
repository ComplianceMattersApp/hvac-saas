-- Repair production drift for internal invoice supplemental schema only.
-- Mirrors 20260605223000_internal_invoices_supplemental_foundation.sql
-- without seeding data or mutating payment/allocation truth tables.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.internal_invoices'::regclass
      and conname = 'internal_invoices_kind_valid_chk'
  ) then
    alter table public.internal_invoices
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.internal_invoices'::regclass
      and conname = 'internal_invoices_primary_has_no_original_chk'
  ) then
    alter table public.internal_invoices
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.internal_invoices'::regclass
      and conname = 'internal_invoices_supplemental_reason_trimmed_chk'
  ) then
    alter table public.internal_invoices
