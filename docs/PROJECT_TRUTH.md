...
- QBO is implemented as optional downstream accounting synchronization for eligible invoices and recorded payments. After EveryStep confirms payment truth, it attempts to create and apply the related QBO Payment. QBO never overrides EveryStep invoice, payment, job, or closeout truth. Voiding an invoice in EveryStep also voids it in QuickBooks (added 2026-08-09); a QBO invoice with payments applied is never auto-voided, because voiding it there would leave the payment unapplied.
- A 2xx response from an accounting provider is not treated as proof the change happened. Void propagation re-reads the record and only records success when the provider confirms it. An unverified success is worse than a recorded failure: success is terminal and retires the row from every queue watching it, while a failure retries.
- Money leaving is instrumented, not only money arriving. Stripe refunds and disputes reach EveryStep by webhook: a full refund reverses the payment, a partial refund is raised for manual allocation, and a dispute reverses only when lost. Voiding an invoice also expires its open Stripe checkout sessions, and a charge that lands after a void is flagged as collected money needing a refund decision rather than silently dropped.
- A scheduled three-way reconciliation compares EveryStep against QuickBooks and Stripe independently of the sync engine, and is report-only. It is the only control that can surface problems EveryStep did not cause — a QBO invoice edited by hand, or a Stripe charge this app never recorded.
...
- Still deferred: customer portal/client hub, ACH, and broader recurring-payment automation. Refunds and disputes are no longer deferred — inbound handling shipped 2026-08-09; what remains deferred there is operator-initiated refunds from inside EveryStep (today a refund is issued in Stripe and EveryStep reacts) and pushing a reversal into QuickBooks.

Closeout and operator paths: [Contractor_Payment_Communications_Closeout_2026-07.md](./ACTIVE/Contractor_Payment_Communications_Closeout_2026-07.md) · [Payment_Controls_Hardening_Closeout_2026-08-09.md](./ACTIVE/Payment_Controls_Hardening_Closeout_2026-08-09.md).
...
| **Route planning engine** | `lib/routing` | Area clustering, day-fit scoring, drive order, arrival windows. Proposals are ephemeral and never persisted. **Live Google Routes drive times** (fetched before planning and passed in as data, so the engine stays pure), falling back to straight-line estimates when unavailable |
...
Not yet supported in this lane: ACH; processor-led refunds; dispute/chargeback handling; manual bank-match certification; or a broad customer payment portal beyond signed invoice links.
