...
Status: CLOSED for current implementation lanes. Recent committed/pushed work restored and locked the customer/location job-intake model, saved service-address correction behavior, ECC retest scheduling parity, Dual Context V1A app/portal access separation, contractor portal proposed-equipment intake capture, portal-to-app signup/reactivation CTA, and Account Device Setup notification-card layout polish. Customers/accounts can have multiple saved service addresses: `customers` remains account/contact/billing identity, `locations` remains saved service-address truth, and jobs continue to store `location_id` plus copied job address snapshot fields. `/jobs/new` visibly supports choosing a saved service address or adding a new service address. Saved service addresses can be edited/corrected; when billing was blank or matched the old service address it stays aligned after correction, and intentionally different billing is preserved. Completed job snapshots are not bulk-rewritten. ECC confirmed Retest Ready now supports Move to Needs Scheduling and Schedule Retest Now; Schedule Retest Now creates the linked retest child and schedules it immediately, while the parent becomes historical/passive after the linked retest child exists. Invoice/payment/cert truth remains separate. Dual Context V1A is locked: portal access and full-app access are separate, portal access survives expired/cancelled app access, active app + portal defaults to app and exposes Partner Work, expired app + portal defaults to portal, expired app only routes to inactive access, and `/jobs/new` uses contractor/portal submission only when explicit portal intake context is present. Contractors can provide proposed equipment during portal intake, but contractor-provided equipment is intake context only; after a real job exists, contractors cannot add/edit/delete canonical job equipment, portal job detail may show equipment read-only and route updates through notes/comments, and internal users retain final control over `job_systems` / `job_equipment`. Portal-to-app CTA is now present without granting app access from portal access: portal-only never-trialed users can start the existing 14-day trial path, expired trial users are directed to paid checkout resume rather than another free trial, cancelled/inactive users are directed to reactivate where safely supported, active app + portal users can open the app, and portal work remains visible while app access is inactive. Account Device Setup notification-card layout is corrected; notification logic and PWA behavior did not change. Deferred/parked: job-only address snapshot correction only if needed later, true default service-location flag only if inferred ordering becomes confusing, broader customer/location IA redesign, contractor post-job equipment edit/delete, portal equipment multi-entry expansion, Install with Permit guided workflow, ECC handoff cleanup, and broader metadata consolidation.

**Payments / Deposits Reporting Foundation Closeout:**
Status: CLOSED for current reporting foundation. Local Checkout return/update diagnosis confirmed the issue was local Stripe CLI webhook forwarding not running, not a core payment bug; with `stripe listen --forward-to localhost:3000/api/stripe/webhook` running and `.env.local STRIPE_WEBHOOK_SECRET` matching the listener secret, local payment confirmation works through the webhook path. Production webhook handling remains Stripe delivery to deployed `/api/stripe/webhook`. Checkout complete now uses `Payment submitted` copy, sends `Return to invoice` and `Back to job` through refresh-aware payment-return state, and no longer needs a separate refresh link. Deposits reporting foundation is complete: settlement table foundation, upsert uniqueness repair, production migration apply/verification, read-only Deposits report/detail/summary CSV/detail CSV, Reports dashboard discoverability for Owner/Admin/Billing, Technician/Dispatcher hide/block behavior, and friendly owner-facing copy. No sync controls are exposed in the UI. Reports/deposits adds no Stripe calls, payment links, invoice/payment/allocation mutations, settlement mutations, QBO behavior, refunds/disputes, or calculation changes. Webhook-confirmed payment/allocation truth remains the only source for invoice paid/balance. Remaining future gate is a controlled production money-flow proof with one real/live paid invoice or existing paid production invoice, one scoped settlement sync, Stripe Dashboard gross/fee/net comparison, report/detail/CSV verification, and payout/bank confirmation when available; that gate is future evidence, not an unresolved blocker to the reporting foundation.

**ECC/Test Workflow Maturity Closeout:**
Status: CLOSED — implemented, smoked by owner, and ready as current ECC field-entry standard. The ECC/Test workflow cleanup is complete for the current intended scope and is documented in [ECC_Test_Workflow_Maturity_Closeout.md](./ECC_Test_Workflow_Maturity_Closeout.md). Duct Leakage, Airflow, and Refrigerant Charge now share a field-first mobile workflow standard: clear test title, Exception first, reason required only when an exception is selected, setup/context collapsed by default, one compact Results area, inline live calculated feedback, no duplicate dashboard/result cards, no side-by-side mobile field entry, and Complete Test returns to the job detail test section. Completion Report now behaves as a print-first report surface, and the main test hub/matrix has been simplified for Equipment, Completion Report, and direct system-chip switching. This maturity pass did not change schema, RLS, payments, Stripe, QBO, SMS, invoice/payment truth, cert closeout truth, ECC evaluator truth, formulas, customer portal behavior, attachment enforcement, or broader service workflow behavior. Known model note: current exception handling uses existing override/exception-compatible plumbing; a dedicated ECC exception truth model remains optional future work only if field use shows reporting/audit needs beyond the current model.

**ECC Guided Workflow Separation Model Lock:**
Status: ACTIVE MODEL LOCK. ECC blocker/failure/retest/handoff/cert-closeout workflow is locked separately in [ECC_Guided_Workflow_Separation_Model_Lock.md](./ECC_Guided_Workflow_Separation_Model_Lock.md). Service follow-up uses Materials Needed / Approval Needed / Other and linked return visits; ECC must not reuse that service workflow. ECC user-facing workflow language is Permit Needed, Failed / Correction Required, Corrections Submitted / Under Review, Retest Ready, linked retest job, ECC handoff, and cert closeout blockers separate from invoice truth. Invoice/payment/no-charge truth must not auto-clear ECC permit, failed, correction, retest, handoff, or cert blockers, and ECC invoice send can remain allowed while cert closeout is blocked.

**Guided Workflow Maturation Closeout:**
Status: CLOSED for current pass. The service-side and ECC-side guided workflow maturation lanes are now implemented, committed, pushed, and locked in [Guided_Workflow_Maturation_Closeout.md](./Guided_Workflow_Maturation_Closeout.md). Core principle: complete what is true today, keep unresolved workflow visible until handled with intent, and continue through the correct linked next job instead of blurring the original visit. Service follow-up now uses Materials Needed / Approval Needed / Other with required reasons, part/approval progress, and linked return child jobs. ECC guided workflow now uses Permit Needed, Failed / Correction Required, Corrections Submitted / Under Review, Retest Ready Requested, internally confirmed Retest Ready, Move to Needs Scheduling, and linked retest child jobs. Parent/original visits become historical/passive after child continuation exists; child jobs become active only through their own scheduling/assignment rules. Invoice/payment/no-charge truth remains separate and does not clear workflow or cert blockers.

**Phase 6J-E2 Closeout (Platform Application Fee Lane, Current Intended Scope):**
Platform application fee lane is closed for current intended scope. Phase A/B foundation is complete with default fee set to `25` basis points (`0.25%`). Phase C checkout wiring is complete. Phase D shared PaymentIntent wiring is complete for saved-card/manual and scheduled-autopay submit path. Phase E/E2 sandbox smoke is complete with both checkout and saved-card/manual evidence: `1750` cents gross charge produced `4` cents application fee in each path. Source-of-truth boundaries remain unchanged: app payment truth remains gross customer payment truth, platform application fee remains Stripe/platform revenue only, no customer-facing surcharge line item is introduced, and invoice paid/balance truth is not distorted. Failed payments remain non-collected. No visit or `next_due_date` mutation occurred. Refunds/disputes remain deferred and ACH remains deferred. Customer payment success redirect polish is complete for the current Checkout return screen, with webhook confirmation still the source of payment truth. Invoice page UX cleanup is the next roadmap lane, followed by customer page IA/UX cleanup. This closeout included no production Stripe action and no schema change.

**Phase 6F-C Closeout (Manual Saved-Card Charge for Issued Invoice):**
Phase 6F implementation is closed in commit `f7fa23fca188029a9a6f38e152a83180b346606e` (`feat(payments): charge saved card manually for issued invoice`) and pushed to `origin/main` with clean post-push tree. Manual one-time saved-card charge now exists for eligible issued invoices and is explicitly not autopay and not Stripe subscription behavior. Source-of-truth lock remains explicit: `tenant_saved_method_payment_attempts` is workflow/audit truth, webhook-created `internal_invoice_payments` is collected-money truth, and allocation occurs after payment truth exists. Fresh sandbox smoke proof: invoice `INV-20260528-1CFFCB88` (`7f79e75b-06b5-4924-bd0c-91b78740f2d7`), attempt `99949838-81f3-442a-9de1-4bc736b4c40b`, payment `3788c9ff-700d-43ab-8339-46e4cbf24ae3`, allocation `2b702b07-690a-4e4d-82f2-6f6ed6e40627`, charge `ch_3Tbxg47itDepDR180C1KhPco`, amount `$17.50`, webhook deliveries HTTP 200, and UI post-state Paid / Balance `$0.00`. Guardrails remained intact: no `tenant_customer_autopay_consents` creation, no `maintenance_agreement_visits` mutation, no `maintenance_agreements.next_due_date` mutation, no Stripe Billing subscription behavior, and no ACH/bank-debit behavior. Validation closeout remained green (`npx.cmd tsc --noEmit`; targeted Vitest matrix 8 files/100 tests; `git diff --check`; `_tmp_*` cleanup).
...
Phase 6H-A audit/model lock is complete as read-only analysis with no code, schema, docs-outside-scope, sandbox data, production data, Stripe charge, or UI mutation. Canonical attempt states remain unchanged (`pending`, `submitted`, `succeeded`, `failed_declined`, `failed_requires_action`, `blocked_precondition`, `retry_scheduled`, `abandoned`). Submit-path lock remains: declines map to `failed_declined`; authentication-required maps to `failed_requires_action`; readiness/scope/invoice/precondition failures remain `blocked_precondition`; stale amount snapshot blocks submit; duplicate in-flight remains blocked; and the 6G-E3 self in-flight exception remains narrow/safe. Webhook lock remains: `charge.failed` is routed; `payment_intent.payment_failed` is not currently routed; failed webhook rows remain non-collected `internal_invoice_payments` truth; failed allocations remain non-counting (`inactive`); and saved-method attempt failure resolution remains webhook-linked where identity is present. Existing surfaces are confirmed: Payments Register separates recorded vs failed rows, Customer Payment History separates Failed Attempts, and invoice workspace has manual saved-card failure banners but no scheduled-autopay-specific attention read model yet. Recommended V1 policy lock is manual-first retry with no automatic infinite retries, `failed_requires_action` pause for same consent/agreement/customer path until operator resolution, `failed_declined` attention with manual retry only after revalidation, and failed autopay not blocking service visits by default. Existing attempt-table fields (`retry_count`, `next_retry_at`, `requires_action_type`, `blocked_reason_code`, `failure_code`, `failure_message`, resolved fields) should be used first with derived read model before additive schema. Customer communications remain deferred (failed-payment email/SMS, self-service/portal card update/retry, retry reminders, notification automation). Locked sequence is 6H-B read model/attention projection, 6H-C invoice workspace read-only attention UI, 6H-D manual retry action with strict guards, 6H-E failed-path sandbox smoke, and 6H-F docs closeout/production readiness gate.
**Phase 6H-B Closeout (Failed Autopay Attention Read Model / Projection Only):**
Phase 6H-B is complete in commit `e2690e2e36c0e40b2797d73bac8985693b18f381` (`feat(payments): add failed autopay attention read model`). Added `lib/business/failed-autopay-attention-read-model.ts` and `lib/business/__tests__/failed-autopay-attention-read-model.test.ts` as server-side read-only projection for account-scoped failed scheduled-autopay attention (`failed_declined`, `failed_requires_action`, meaningful `blocked_precondition`) with category/status/action counts. Truth boundaries remain unchanged: `tenant_saved_method_payment_attempts` is attempt/attention truth, `internal_invoice_payments` is payment-event truth, `internal_invoice_payment_allocations` is allocation truth, Stripe remains processor/payment-method truth, and visits/`maintenance_agreements.next_due_date` remain operational truth with no payment-driven mutation.
**Phase 6H-C Closeout (Invoice Workspace Read-Only Failed-Autopay Attention UI):**
Phase 6H-C is complete in commit `5b383e842a62d0cc95f7b1d90ca3865b735f5e87` (`feat(payments): show failed autopay attention on invoice workspace`). Invoice workspace now loads the failed-autopay attention read model server-side and displays a read-only "Failed Scheduled Autopay Attention" panel for internal operator visibility (open items, category, attempt status, timestamp, recommended operator action, and safe context metadata). This is visibility-only with no retry action, no Stripe behavior change, no schema change, no production enablement change, no customer message flow launch, and no payment/allocation/invoice/visit/next_due mutation outside existing webhook payment truth behavior. 6H-D manual retry, 6H-E failed/`requires_action` sandbox smoke, and customer communication/portal update-card flows remain deferred.

**Phase 6H-D Closeout (Manual Retry for Failed Scheduled Autopay):**
Phase 6H-D is complete in commit `c3ea465987ac138822b01c914839c6ec62a696fa`. Manual retry helper/action/UI affordance is now implemented for retry-eligible failed scheduled-autopay attention through sanctioned invoice-workspace pathing. Guardrails remained intact: retry creates a new attempt row in `tenant_saved_method_payment_attempts`, does not directly write `internal_invoice_payments`, does not directly write `internal_invoice_payment_allocations`, does not directly mutate invoice paid/balance state, and does not mutate visits or `maintenance_agreements.next_due_date`. Source-of-truth remains locked: `tenant_saved_method_payment_attempts` = attempt/attention truth, `internal_invoice_payments` = payment-event truth, `internal_invoice_payment_allocations` = allocation truth, Stripe = processor/payment-method truth. Successful retry-settlement smoke remains optional/future and only via safe sanctioned saved-card path.

**Phase 6H-E Closeout (Declined-Path Manual Retry Smoke, Sandbox-Only):**
6H-E cleanup/stabilization (E2/E3/E4) completed with sanctioned fixture cleanup and consent/method coherence restoration, with no code/docs/schema commits in cleanup lane. 6H-E5 completed in commit `d5dd4f918b0178157dbcb54edd5f9203a9b943e3` by adding sanctioned billing-anchor path (`linkBillingAnchorJobFromForm`) without invoice/payment/allocation/Stripe/visit/next_due side effects. 6H-E5B completed in commit `b12e1cc6e934d2d4c0203104d3e393387890619c` by blocking cancelled/canceled/closed/void/voided/archived/soft-deleted jobs as billing anchors. 6H-E6 declined-path smoke is complete/validated for customer `ad18fa80-2817-476b-8fca-bdcf4ff3c3d6`, billing period `c89c4c36-a842-40e2-9b20-745dce4b959c`, job `1a52288c-78ae-4e79-9472-d00ed928f32f`, and issued invoice `INV-20260529-DDC200B6` (`3d5edb10-8695-42ab-a133-54bd64e4a2a0`): initial scheduled attempt `6d9120a4-d571-41a0-8ad3-b5172ac39275` and manual retry attempt `980c90c2-745f-470f-aa86-e2ba5b30fbc0` both resolved `failed_declined` (retry count `1`, failure "Your card was declined."). Stripe listener captured `charge.failed` and `payment_intent.payment_failed` with webhook HTTP 200. Post-state remained correct: one failed `internal_invoice_payments` row (non-collected), one inactive `internal_invoice_payment_allocations` row (non-counting), invoice summary unpaid (paid `0`, balance `1750`), declined retry still visible in attention, and no direct invoice paid mutation or direct payment/allocation writes from retry action. Customer email/SMS/portal update-card flows remain deferred. Next required visibility lane before production-grade autopay rollout is Failed Payment Alert + Reconciliation Queue so failed payments do not occur silently.

**Phase 6I-A Closeout (Failed Payment Alert + Reconciliation Queue Model Lock, Docs-Only):**
6I-A model lock is complete as docs/audit-only with no code changes, no migrations, no sandbox/production mutation, no Stripe actions, and no read-model/alert/queue implementation. Preflight confirms clean synced branch state (`HEAD == origin/main`) at `10aa8983645b59f46f3a5a969f79298b9694b756` with latest commit `polish(tests): standardize mobile ECC console layout`; grep did not find commit message `docs(payments): close failed autopay retry smoke`, but failed-autopay retry closeout is clearly reflected in ACTIVE docs (6H-B/6H-C/6H-D/6H-E6). V1 open failed-payment attention definition is locked to `tenant_saved_method_payment_attempts.attempt_kind = scheduled_autopay` where `attempt_status` is `failed_declined`, `failed_requires_action`, or meaningful `blocked_precondition`, and where items are not resolved by `resolved_internal_invoice_payment_id` and not terminally closed by `succeeded`/`abandoned`; queue relevance should prioritize issued/non-void invoices with positive balance due. Visibility rule is locked: failed payments must not occur silently, and account-level/operator-level visibility is required before production-grade autopay rollout. Source-of-truth lock remains unchanged: `tenant_saved_method_payment_attempts` = attempt/attention truth, `internal_invoice_payments` = payment-event truth, `internal_invoice_payment_allocations` = allocation truth, Stripe = processor/payment-method truth, invoice paid/balance = projection from collected payment truth only, visits/`maintenance_agreements.next_due_date` remain operational truth and are not payment-mutated, failed payment rows are non-collected truth, inactive allocations do not count toward paid balance. Recommended first alert surface is Owner/Admin/Billing-only failed-payment attention card on Ops or admin/dashboard with open count and category breakout when practical, linking to a dedicated reconciliation queue; Payments Register remains financial payment-event truth and is not attempt-attention truth. Recommended queue posture is separate from Payments Register, read-model-backed from attempt truth with invoice/payment enrichment, with rows for customer, invoice number, balance due, failure category/reason, last attempt time, retry eligibility, recommended action, and invoice workspace link. V1 actions are open invoice workspace, open customer, and copy failure context; retry remains invoice-workspace action; `mark acknowledged/reviewed` is deferred unless explicitly approved. Explicit deferrals: failed-payment email/SMS, portal update-card, customer self-service retry, automated retry scheduling/policies, successful retry-settlement smoke unless safely executable through sanctioned saved-card flows, and `payment_intent.payment_failed` routing unless explicitly approved in this phase. Known blockers/risks are locked: scheduled-autopay close semantics are not fully explicit in current write paths (read model closure expects `resolved_internal_invoice_payment_id` or terminal status), webhook resolver appears scoped to `manual_saved_method` attempts so scheduled-autopay resolution should be made explicit before queue close/resolution actions, failure truth currently relies on `charge.failed`, and docs/history reconciliation should be explicit because branch HEAD differs from expected docs-closeout commit message. Proposed sequence lock: 6I-B failed-payment reconciliation read model, 6I-C Owner/Admin/Billing alert card, 6I-D reconciliation queue UI, 6I-E sandbox smoke plus docs closeout.

**Phase 6I-E Closeout (Failed Payment Reconciliation Visibility, Docs-Only):**
6I-B/6I-C/6I-D are complete and 6I-E smoke is now complete after switching to the fixture owner account context (Service Account). The visibility chain is proven end-to-end: failed-payment items appear in account-level failed-payment reconciliation read model truth; Ops failed-payment alert is visible for financial-authority users; alert links to `/reports/failed-payments`; queue renders failed-payment rows; queue drill-in links to invoice workspace; and invoice workspace shows detailed Failed Scheduled Autopay Attention. Smoke evidence in fixture owner context confirmed queue open count `2`, balance at risk `$35.00`, and two declined rows for `INV-20260529-DDC200B6`, with drill-in to `/jobs/1a52288c-78ae-4e79-9472-d00ed928f32f/invoice` showing issued/unpaid plus open declined attention items. Alert and queue remain read-only visibility surfaces: no retry action there, no acknowledge/review/resolve queue actions, no customer email/SMS, and no portal update-card or self-service retry flow in this lane; retry remains invoice-workspace-only. Source-of-truth boundaries remain unchanged: `tenant_saved_method_payment_attempts` = attempt/attention truth, `internal_invoice_payments` = payment-event truth, `internal_invoice_payment_allocations` = allocation truth, Stripe = processor/payment-method truth, Payments Register = payment-event history/reporting truth, and Failed Payment Reconciliation Queue = unresolved attempt/attention visibility. Invoice paid/balance remains collected-payment projection only, failed rows remain non-collected truth, inactive allocations remain non-counting, visits/`maintenance_agreements.next_due_date` remain operational truth, and there is no schema change, Stripe behavior change, production enablement change, or payment/allocation mutation beyond existing webhook behavior.

**Phase 6G-E4 Closeout (Fresh Scheduled Autopay Submit Smoke, Docs-Only):**
Phase 6G-E4 passed after the 6G-E3 self-attempt revalidation fix in commit `c7329a8a9b19d392f6dd7196ca7145f86d62e713`. Sandbox-only smoke used owner `9e82acca-c271-41bc-89af-396f37c1990c`, customer `ad18fa80-2817-476b-8fca-bdcf4ff3c3d6`, invoice `63e28e1c-1be9-43bb-923d-940d80887cb2` (`INV-20260528-9D731258`), job `91e31a74-cc1b-4585-8dc3-6812351fbbdf`, consent `3c24c9e6-6f78-4fe4-8619-9094782827bb`, fresh pending attempt `67dc6700-83d9-4dd0-8af1-d8ae931db14a`, Stripe PaymentIntent `pi_3Tc6bF7itDepDR181kf2LE2p`, charge `ch_3Tc6bF7itDepDR181J27CvPs`, and webhook event `evt_3Tc6bF7itDepDR181B08QHZd`. 6G-B returned the invoice eligible, 6G-C created the fresh pending scheduled_autopay attempt, and 6G-D submitted it successfully. Webhook truth created exactly one `internal_invoice_payments` row and exactly one `internal_invoice_payment_allocations` row; invoice projection became paid and balance dropped to `$0.00`. UI verification after refresh showed Issued / Paid / Paid `$17.50` / Balance `$0.00` with one recorded Stripe payment row. Guardrails held: no direct payment/allocation writes by submit helper, no manual invoice paid mutation, no maintenance visit or `next_due_date` mutation, no invoice issue/send/email side effect, no payment-link creation, no production access, and no code changed during the smoke. All requested validations passed. Recommended next lane is Phase 6G-F docs closeout, then Phase 6H failed-payment retry and attention workflow expansion.
Scope is locked to audit/model/planning only. No implementation, migrations, scheduler jobs, Supabase mutation, Stripe calls, webhook behavior changes, or commit authorization are included.

Locked boundaries:
- Scheduler is attempt orchestration truth only.
- `tenant_saved_method_payment_attempts` is workflow/audit truth only.
...
- Failed scheduled autopay produces attention only and does not mutate `maintenance_agreement_visits` or `maintenance_agreements.next_due_date`.

Locked eligibility model:
- issued, non-void invoice with positive balance due.
- same account/customer/payment-profile/consent scope.
- linked billing period (if present) not cancelled; linked agreement (if present) active/autopay-eligible.
- active saved method + enabled valid consent.
- connected account ready/matching.
- no in-flight `scheduled_autopay` attempt for same invoice.
- enforce consent max amount when configured.
- stale/disconnected/invalid method/profile blocked.
- re-check status/balance before submit; already-paid invoice blocks as `blocked_precondition`.

Locked scheduler posture:
- first slice is manual/internal runner only.
- dry-run and commit modes.
- no cron in first slice.
- evaluate issued invoices only (exclude drafts), with due-today and overdue first.
- batch by `account_owner_user_id` with per-account failure isolation.

Locked attempt lifecycle/idempotency:
...
- `eligible` remains dry-run/read-model output only (not persisted).
- idempotency key: `scheduled_autopay:account_owner_user_id:invoice_id:cycle_key:ordinal`.
- no duplicate in-flight attempts and no duplicate PaymentIntent per attempt.
- amount/balance snapshots at create with pre-submit revalidation.

Locked charge model/retry semantics:
- scheduled submit must reuse shared manual saved-card charge service path.
- PaymentIntent metadata must include account/customer/invoice/attempt and optional billing period/agreement identifiers.
- only pre-webhook transition allowed is `submitted`; payment rows remain webhook-only writes.
...
Locked reporting/security/dry-run/schema:
- attempt history is separate from recorded payment totals.
- failed attempts never count toward collected totals.
- consent/scheduler/retry controls are Owner/Admin/Billing only with server-side financial checks.
- dry-run is mandatory first slice and must emit evaluated/eligible/blocked/in-flight and snapshot telemetry with explicit no Stripe/no writes.
- no schema change is required for 6G-B/6G-C base slices; `eligible` stays non-persisted; optional future additive fields only (`scheduler_run_id`, `cycle_key`, `last_blocked_reason_at`).

Locked risks and sequence:
- risks: eligibility-submit race, duplicate attempts, requires_action loops, cross-tenant failure cascade, missing billing-period linkage, attention UX gaps.
- sequence: 6G-B eligibility read model + dry-run tests; 6G-C attempt creation only; 6G-D shared-path submission; 6G-E sandbox smoke; 6G-F docs closeout; 6H retry/attention expansion.

Execution confirmation: docs-only model lock closeout; no implementation/data mutation occurred.
...
The bookkeeping-ready payment register and allocation model is locked in [Financial_Ledger_Payments_Register_V1_Model_Spec.md](./Financial_Ledger_Payments_Register_V1_Model_Spec.md). **Payments Register V1A/V1B are now implemented (commit `c9dc763`)**: read-only `/reports/payments` with Owner/Admin/Billing gating, recorded/failed separation, and method taxonomy (ACH hidden); filtered CSV export at `/reports/payments/export` with proper escaping and financial-export gating. **Payments Register V1C (Customer Profile Payment History) is now implemented and browser smoke passed (commit `55dab8c`)**: read-only payment history section on `/customers/{id}` page with Owner/Admin/Billing gating (Dispatcher/Technician/Contractor blocked by default), recorded/failed/other status separation, method taxonomy (ACH hidden), and per-payment details (amount, status, method, date, invoice, job link, reference, notes). The card's Open Payments Register link is confirmed to open the full register with customer filter, and full register CSV export remains available. Register/history reads from `internal_invoice_payments` current truth only; no mutations, corrections, allocations, schema changes, or Stripe/Supabase/prod changes. Future register work must separate Payment Register Entry, Payment Allocation, Invoice Payment Projection, and Failed Payment Attempt. Stripe remains processor truth for Stripe transactions; Compliance Matters remains tenant financial operating truth for all money received. ACH, QBO sync, refunds/disputes, platform fees, saved cards, full accounting ledger, customer portal self-service, and recurring billing execution remain deferred.
Service Plan Billing Foundation Phase 2 model lock is documented in [Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md](./Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md): first posture is billing-period commercial truth linked to normal internal invoices and existing invoice payment truth, with auto-charge/autopay/subscription behavior deferred.
Optional internal billing guardrail is locked: internal invoice/payment attachment is optional by `billing_mode`, tenant setup, and future service-plan billing configuration; jobs/work orders/service plans/maintenance visits/visit counts/next-due workflows must remain operational without internal payment rows; external-billing/off-platform tenants remain fully operational without internal payment attachment; future Service Plan Billing Period posture must support internal invoice-backed, external/off-platform, manual, no-charge, waived, and not-billed-through-Compliance-Matters paths; payment status may inform warnings/reporting but must not hard-block first-posture operations unless explicitly designed later; and payment truth must not attach directly to `maintenance_agreement_visits`.

**Payments Reversal / Correction Foundation (Phase 3A) closeout:**
Phase 3A is implemented as an additive correction slice on `internal_invoice_payments`: reversal audit fields (`reversed_at`, `reversed_by_user_id`, `reversal_reason`) are now present; manual/off-platform recorded rows can be reversed by Owner/Admin/Billing with required reason; failed and already-reversed rows are blocked; Stripe/online rows are read-only in this correction flow and explicitly do not trigger refund/dispute/provider API behavior. Reversal is non-destructive (row preserved and marked reversed with audit metadata), and projection safety remains locked: only `recorded` rows count toward invoice paid/balance and collected totals. Deferred register remains unchanged (allocations, service-plan billing periods execution, customer portal, QBO, ACH, refunds/disputes, saved cards/autopay, partial payments, receipt automation, platform-fee execution).

**Allocation Compatibility Foundation (Phase 4A) closeout:**
Phase 4A is implemented as a compatibility-only foundation in commit `a0a2d23`: no allocation schema/table exists yet and no allocation rows are persisted. Existing invoice-bound payment projection now routes through allocation-compatible helper semantics only, preserving recorded-only collected truth and keeping failed/reversed rows excluded from collected totals. No Service Plan billing period behavior, `maintenance_agreement_visits`, payment recording behavior, Stripe checkout/webhook behavior, customer portal, QBO, ACH, refunds/disputes, saved cards/autopay, or partial payments behavior changed in this slice.

**Allocation Schema Model Lock (Phase 4B, docs/model only):**
First explicit allocation table name is locked to `internal_invoice_payment_allocations`. First source is locked to `source_internal_invoice_payment_id` referencing `internal_invoice_payments.id`. First target is invoice-only via `target_invoice_id`; service-plan billing period targets (including `target_service_plan_billing_period_id`) and customer-credit targets are future expansion only. First posture is one source payment to one invoice allocation enforced by unique `source_internal_invoice_payment_id`, with statuses locked to `active`, `inactive`, `reversed`, and `voided`. Counting lock is explicit: only `active` allocations count toward invoice collected totals; inactive/reversed/voided do not count. Any future `counts_toward_collected_totals` field must be omitted or constrained to status-consistent truth.

**Phase 4C boundary lock:**
Next implementation slice is additive table + RLS + indexes + tests only, with no UI, no projection switch/read-path switch, no payment recording changes, no Stripe/webhook behavior changes, and no Service Plan billing behavior changes.

**Phase 4C closeout (Explicit Invoice Payment Allocation Table Foundation):**
Phase 4C is complete as additive schema foundation in migration `20260526130000_internal_invoice_payment_allocations_foundation.sql`. Table `internal_invoice_payment_allocations` is now present with first-posture invoice-only target (`target_invoice_id`) and one-source-to-one-allocation uniqueness (`source_internal_invoice_payment_id`). Allocation statuses are constrained to `active`, `inactive`, `reversed`, and `voided`. `counts_toward_collected_totals` was intentionally not added; future countability remains status-derived from `allocation_status = 'active'`. Account-scoped RLS SELECT/INSERT/UPDATE policies are present with no DELETE policy, and write-time scope checks enforce source-payment and target-invoice account alignment for first posture. No backfill was performed, no runtime flows write allocation rows yet, and no projection/read-path switch was introduced. Existing invoice-bound collected-payment truth remains unchanged, and no UI/payment recording/Stripe checkout/webhook/Service Plan billing behavior changed in this slice.

**Phase 4D closeout (Allocation Population / Backfill / Write Strategy, docs/model only):**
Allocation rows are locked to future one-to-one population from `internal_invoice_payments`, with `source_internal_invoice_payment_id` as the allocation idempotency key. Locked first mapping is `recorded -> active`, `pending/failed -> inactive`, and `reversed -> reversed`. `allocated_amount_cents` must preserve source `amount_cents` exactly (including signed/zero parity), and `target_invoice_id` must equal source payment `invoice_id`. Failed/reversed source rows should still receive allocation rows for lifecycle completeness but remain non-counting. Projection remains locked on compatibility helper semantics until parity is proven, and no projection/read-path switch is allowed yet. Backfill posture is locked to idempotent/retryable behavior; runtime allocation writers are locked to centralized helper posture; manual payment and Stripe webhook dual-write remain separate implementation slices; historical backfill must run only after runtime write strategy is implemented/locked; and production dormant schema migration planning/apply requires explicit approval before any runtime writer ships.
...
**Phase 4E closeout (Production Dormant Allocation Migration Catch-up):**
Phase 4E production dormant schema catch-up is complete on production ref `ornrnvxtwwtulohqwxop`. Applied migrations in order were `20260526110000_internal_invoice_payments_reversal_audit_foundation.sql` then `20260526130000_internal_invoice_payment_allocations_foundation.sql`. Production verification confirmed reversal audit schema on `internal_invoice_payments` (`reversed_at`, `reversed_by_user_id`, `reversal_reason`, reversal index) and confirmed `internal_invoice_payment_allocations` table contract (required columns/constraints/indexes, RLS enabled, SELECT/INSERT/UPDATE policies present, no DELETE policy, scope assertion trigger/function present). Forbidden/deferred columns remain absent (`counts_toward_collected_totals`, `target_service_plan_billing_period_id`, customer-credit target fields). Allocation row count is `0`, no backfill ran, no runtime allocation writers exist yet, and no projection/read-path switch was introduced. No payment recording/manual payment behavior, Stripe checkout/webhook behavior, UI behavior, Service Plan Billing behavior, QBO/ACH/refunds/disputes/saved cards/autopay/partial payments/receipt automation/platform fee behavior, customer portal behavior, or service-plan automation behavior changed in this closeout.

**Phase 4F closeout (Centralized Allocation Write Helper Foundation):**
Phase 4F centralized allocation write helper foundation is complete. A centralized helper now exists to create/update one persisted allocation row from one `internal_invoice_payments` row, using `source_internal_invoice_payment_id` idempotency and invoice-only target posture. Implemented mapping remains `recorded -> active`, `pending/failed -> inactive`, and `reversed -> reversed`, with source `amount_cents` preserved exactly including signed/zero parity. The helper is not wired into runtime payment flows yet. No manual payment dual-write exists yet, no Stripe webhook dual-write exists yet, no historical backfill ran, and no projection/read-path switch was introduced. No UI behavior changed, and no payment recording/manual payment/Stripe checkout/webhook/Service Plan Billing/QBO/ACH/refunds/disputes/saved cards/autopay/partial payments/receipt automation/platform fee/customer portal/service-plan automation behavior changed in this phase. Next slice remains Phase 4G manual payment dual-write, or a narrow Phase 4G-A helper smoke/parity pass if needed before runtime wiring.

**Phase 4G closeout (Manual Payment Dual-Write):**
Phase 4G manual/off-platform dual-write is complete for manual payment recording and manual payment reversal actions only. Manual recorded payments now invoke centralized allocation upsert to create/update allocation rows keyed by `source_internal_invoice_payment_id`, and manual reversal now invokes centralized allocation upsert after payment-row reversal to transition allocation status to `reversed` for the same source payment. Payment rows remain authoritative; allocation dual-write failures are non-blocking for manual payment record/reversal success. No allocation deletes or duplicate allocation rows were introduced in this slice. Stripe webhook dual-write remains deferred to Phase 4H. Historical backfill remains deferred. Projection/read path remains unchanged and still uses compatibility-helper/payment-row truth. No UI, Stripe checkout/webhook, Service Plan Billing Period, QBO, ACH, refunds/disputes, saved cards/autopay, partial payments, receipt automation, platform fee, customer portal, or service-plan automation behavior changed in this phase.

**Phase 4H closeout (Stripe Webhook Dual-Write):**
Phase 4H Stripe webhook dual-write is complete for Stripe webhook payment handlers. Successful Stripe tenant invoice payment rows now invoke the centralized allocation helper and create/update `active` allocation rows, while failed Stripe tenant invoice payment rows now invoke the centralized allocation helper and create/update `inactive` allocation rows. Idempotent/replayed Stripe events now attempt allocation upsert against the resolved existing payment row without changing existing Stripe payment idempotency or duplicate-protection behavior. Allocation helper failure remains non-blocking after payment-row success, and payment row truth remains authoritative. Existing Stripe event routing remains unchanged. Projection/read path remains unchanged and still does not read persisted allocations. Historical backfill remains deferred. No UI behavior changed, and no Service Plan Billing Period, QBO, ACH, refunds/disputes, saved cards/autopay, partial payments, receipt automation, platform fee, customer portal, or service-plan automation behavior was added in this phase.

**Phase 4I-B closeout (Sandbox Historical Allocation Backfill + Parity Verification):**
Phase 4I-B sandbox historical allocation backfill is complete as docs-only closeout. Backfill/parity work was performed against sandbox ref `kvpesjdukqwwlgpkzfjm`, and production ref `ornrnvxtwwtulohqwxop` was not queried or mutated. Supabase CLI temp state was mixed, so data mutation execution used an explicit sandbox URL/ref gate rather than CLI temp state. Preflight baseline was payment rows `3`, allocation rows `0`, missing allocation rows `3`, statuses `recorded 2` and `reversed 1`, with no unexpected statuses, no required-field gaps, no missing invoice/account/job mismatch, and no duplicate allocation sources. Backfill attempted `3` rows and returned `3` rows, with allocation statuses `active 2` and `reversed 1`. Post-backfill parity verified payment rows `3`, allocation rows `3`, missing allocation rows `0`, status mapping mismatches `0`, payload mismatches `0`, duplicate allocation sources `0`, per-invoice parity mismatches `0`, global recorded payment cents `10134`, global active allocation cents `10134`, global parity matches `true`, and reversed allocations active count `0`. Runtime boundaries remained preserved: no projection/read-path switch, no UI/report behavior changes, no manual payment behavior changes, no Stripe webhook behavior changes, and no production mutation. Validation snapshot remained green: payment allocation + internal invoice payment tests `38` passed, payments register + invoice ledger tests `15` passed, `npx.cmd tsc --noEmit` passed, and branch was clean/synced.

**Phase 4I-C closeout (Production Historical Allocation Backfill Preflight + No-Op Decision):**
Phase 4I-C production historical allocation backfill preflight is complete as docs-only closeout. Production ref `ornrnvxtwwtulohqwxop` was confirmed, trusted production read access was confirmed, and SELECT-only audit execution was used with `mutation_performed=false`. Preflight metrics returned production payment rows `0`, production allocation rows `0`, missing allocation rows `0`, payment status breakdown `{}`, unexpected statuses `[]`, required field gaps `0`, missing invoice count `0`, account mismatch count `0`, job mismatch count `0`, duplicate allocation sources `0`, status mapping mismatches `0`, payload mismatches `0`, per-invoice parity mismatch count `0`, global recorded payment cents `0`, global active allocation cents `0`, global parity matches `true`, and reversed allocations active by mistake `0`. Production backfill is therefore not needed because there are no production payment rows. Runtime boundaries remain preserved: no projection/read-path switch has occurred, payment-row truth remains authoritative, and allocation table posture remains ready for future rows through manual and Stripe dual-write.

**Phase 5B closeout (Service Plan Billing Period Model Lock):**
Phase 5B model lock is complete as docs/model only. Table name is locked to `maintenance_agreement_billing_periods` while product/UI language remains Service Plan Billing Period. Source-of-truth boundaries are locked: Maintenance Agreement (recurring obligation), Maintenance Agreement Visit (operational visit/link/counting), Billing Period (commercial coverage window), Internal Invoice (billed commercial truth), Internal Invoice Payment (collected money truth), and Payment Allocation (payment-to-invoice relationship truth). Paid/unpaid billing state is derived display truth only and cannot become operational truth. First posture is locked to optional linkage from one billing period to one normal existing job-scoped internal invoice; no internal-invoice model expansion beyond required `job_id`, no invoice auto-generation in this foundation slice, and invoice/payment linkage remains optional. Required first-slice field contract is locked to identity/scope/date/cadence/amount/currency/posture/status/internal-invoice-reference/external-reference-and-reason/audit fields; forbidden fields are locked to payment/allocation/visit-link IDs, visit-count and next-due mutation fields, operational blocking flags, and direct Stripe/subscription/QBO IDs. Lifecycle statuses are locked to `draft`, `pending_billing`, `invoice_linked`, `externally_billed`, `no_charge`, `waived`, `not_billed`, `cancelled`; billing posture values are locked to `internal_invoice`, `external_off_platform`, `manual`, `no_charge`, `waived`, `not_billed_through_compliance_matters`; derived payment display states are locked to `not_invoice_backed`, `invoice_draft`, `unpaid`, `partially_paid`, `paid`, `invoice_void`, and `payment_attention` (read-model only). Invoice linkage rules are locked to same-account/customer scope with service-plan-originated/job-related invoice preference when available and no multi-period claim of a single invoice in first posture; allocations remain invoice-targeted and do not directly target billing periods yet. External/off-platform/manual/no-charge guardrails are locked: no fake CM payment rows, no-charge/waived/not-billed never treated as collected money, external references/notes/status metadata allowed. Operational guardrails are locked: jobs/work orders/visits do not require billing period, visit counting does not require invoice/payment, billing-period status does not mutate `maintenance_agreement_visits`, payment status does not advance `next_due_date`, unpaid may inform warnings/reporting only, and non-internal-billing tenants remain supported. Phase 5C acceptance criteria are locked to additive table-only foundation with strict RLS/account scope and same-account agreement/customer/invoice checks, with no UI, no invoice generation, no payment behavior changes, no projection switch, and no service-plan visit/count behavior change.

**Phase 5C closeout (Service Plan Billing Period Schema Foundation):**
Phase 5C schema foundation is complete as schema/tests/docs only. Additive migration `20260526150000_maintenance_agreement_billing_periods_foundation.sql` is now present and adds table `maintenance_agreement_billing_periods` for first-posture Service Plan Billing Period commercial coverage records. The migration enforces locked first-posture constraints (coverage-window validity, nonnegative amount, lowercase ISO currency format, lifecycle status set, billing posture set), duplicate-coverage-window prevention, and optional one-claim-per-internal-invoice uniqueness when `internal_invoice_id` is present. Same-account integrity is enforced by trigger/function checks for maintenance agreement account alignment, optional customer-to-agreement match, and optional internal invoice account/customer scope consistency where available in current invoice model. Account-scoped RLS is enabled with SELECT/INSERT/UPDATE policies and no DELETE policy. Validation completed: focused schema foundation test, maintenance-agreements suite, relevant payment allocation/internal-invoice tests, `npx.cmd tsc --noEmit`, `git diff --check`, and successful local migration reset/apply (`supabase db reset --local --no-seed --yes`). Runtime boundaries are preserved: no UI, no invoice generation, no payment behavior changes, no allocation projection/read-path switch, no Stripe checkout/webhook behavior changes, and no service-plan operational behavior changes. Billing periods remain non-blocking for jobs/work orders/visits/visit counting/next-due workflows. Sandbox/production migration apply remains separate and was not executed in this phase.

**Phase 5C-2 closeout (Production Dormant Billing Period Migration Apply):**
Migration `20260526150000_maintenance_agreement_billing_periods_foundation.sql` applied to production (`ornrnvxtwwtulohqwxop`) on 2026-05-26. Linked ref returned to CMTest sandbox `kvpesjdukqwwlgpkzfjm` after apply. Table `public.maintenance_agreement_billing_periods` verified in production: all 20 required columns present, no forbidden fields, all constraints/indexes/RLS/policies/triggers/functions confirmed against sandbox-verified schema, row count `0`. No billing period rows created, no invoice generation, no backfill. No UI, payment, Stripe, allocation, projection, or service-plan operational behavior changed. Phase 5C is fully closed across repo, sandbox, and production. Next slice is Phase 5D read-model planning/foundation.

**Phase 5D-B closeout (Service Plan Billing Period Read-Model Helper Foundation):**
Added read-only helper module `lib/maintenance-agreements/billing-period-read-model.ts` for account/agreement/customer list helpers and pure coverage/posture/lifecycle/amount/payment-state derivation. Invoice-backed rows derive payment display state from current internal invoice truth and recorded payments only; `payment_attention` does not inflate paid totals. The helper does not query payment allocation rows directly and keeps forbidden payment, allocation, visit, next-due, or blocking fields out of the read model. No UI, mutation, invoice generation/linking action, payment behavior change, allocation read-path switch, or service-plan blocking was introduced. Phase 5D-B is complete; next slice remains Phase 5D-C.

**Phase 5E-B closeout (Customer Profile Read-Only Billing Period Visibility):**
Customer-profile-only read-only Billing Periods visibility is now present inside each internal Maintenance Agreement card on `app/customers/[id]/page.tsx`. The slice is display-only: no billing-period mutations, no invoice generation/linking, no payment/Stripe/allocation/projection behavior changes, and no service-work blocking were introduced. Billing periods remain non-blocking for work orders, visits, next due date, and visit counting. Phase 5E-B is complete; next slice remains Phase 5E-C.

**Phase 5F-A2 closeout (Billing Period Manual Mutation Model Lock):**
Manual billing-period mutation is locked to start customer-profile-only inside existing Maintenance Agreement cards. Mutation authority is Owner/Admin/Billing financial authority; read visibility remains broader/internal under existing Maintenance Agreement visibility. First mutation slice is create/edit/cancel only, with no delete; cancellation uses `billing_period_status = cancelled`. Required manual fields are coverage start/end, billing cadence, amount, currency, billing posture, and lifecycle status, with account/customer/agreement derived from scoped context. Coverage validation is locked to valid dates, end date >= start date, exact duplicate window rejection, overlap rejection for non-cancelled rows, and cancelled rows not blocking future windows. No invoice generation/linking, payment rows, allocation rows, Stripe calls, projection/read-path changes, or work-blocking behavior are introduced by the first mutation slice. Phase 5F-A2 is a model lock only; implementation remains deferred.

**Phase 5F-B1 closeout (Manual Billing Period Server Actions Foundation):**
Manual billing-period server actions are complete and server-action only; no UI was added in this slice. Mutation authority is enforced to Owner/Admin/Billing through the active internal-user and financial-access gate. Create/edit/cancel actions validate customer-profile/agreement scope, required coverage fields, posture/status rules, duplicate/overlap windows, and cancel-by-status-only behavior. Delete remains forbidden; cancellation remains the only end-state and uses `billing_period_status = cancelled`. No invoice generation/linking, payment rows, allocation rows, Stripe calls, projection/read-path changes, or service-plan operational blocking were introduced. Validation snapshot: billing-period action tests passed, billing-period read-model tests passed, maintenance-agreements suite passed, financial-access suite passed, `npx.cmd tsc --noEmit` passed, and `git diff --check` passed.

**Phase 5F-B2 closeout (Customer Profile Billing Period UI Wiring):**
Customer-profile billing-period mutation UI wiring is complete inside existing Maintenance Agreement cards. Mutation controls are customer-profile-only and use the already-tested server actions for create, edit, and cancel. Owner/Admin/Billing controls are shown only when the clean financial-access signal is available; read-only viewers remain read-only. Delete is not exposed. No invoice generation/linking, payment rows, allocation rows, Stripe calls, projection/read-path changes, or service-plan operational blocking were introduced. Browser smoke was attempted, but the available session was not authorized for the target customer profile, so the smoke path remained blocked by access rather than implementation.

**Phase 5F-B3 closeout (Sandbox Billing Period UI Smoke):**
Phase 5F-B3 sandbox UI smoke is complete on sandbox ref `kvpesjdukqwwlgpkzfjm` for customer `ad18fa80-2817-476b-8fca-bdcf4ff3c3d6` and maintenance agreement `454b3737-fa39-46be-8925-45131a571693`. Customer-profile create/edit/cancel workflow passed. Cancellation remained status-based and non-destructive, and the cancelled row remained visible as history. Exact same-window reuse after cancellation remains blocked by current model/schema behavior and is treated as a future model decision rather than a smoke failure. Adjacent replacement billing period creation succeeded. No invoice generation/linking, no internal invoice payment creation, no payment-allocation creation, no Stripe/webhook behavior, no `maintenance_agreement_visits` mutation, and no `next_due_date` mutation occurred. Billing periods remain non-operational and do not block work orders, visits, visit counting, or next due behavior. Commit `d751b23` fixed async Supabase server-client resolution in billing-period actions and added regression coverage.

**Phase 5G-A2 closeout (Billing Period Invoice Linkage Model Lock):**
Phase 5G-A2 is complete as docs/model only. First invoice relationship posture is manual link to an existing internal invoice. Invoice generation from billing periods is deferred. Invoice schema expansion is deferred. Billing-period invoice line-item generation is deferred. Linking remains relationship-only in first posture: no payment rows, allocation rows, Stripe calls, payment-link creation, invoice issue/send behavior, or invoice email behavior are introduced. Billing-period paid state remains derived display from existing invoice/payment truth only. Billing periods remain non-operational and non-blocking for work execution. Manual link eligibility is locked to Owner/Admin/Billing authority, same-account billing period/invoice scope, non-cancelled billing period, null `internal_invoice_id` before link, non-void invoice, invoice not already claimed by another billing period, customer-scope match where invoice customer scope exists, and required invoice-job linkage to the same maintenance agreement via `maintenance_agreement_visits` in first posture. Manual unlink/correction posture is locked to Owner/Admin/Billing authority with required reason, non-destructive behavior, no invoice/payment/allocation row mutation, clear `internal_invoice_id` only, preserve prior invoice/payment history, and return billing-period lifecycle status to `pending_billing` unless later model approval changes that rule. Status/display lock remains: link sets billing-period status `invoice_linked`; paid/partial/unpaid remains derived; linked invoice void should surface `invoice_void` display state without auto-mutating billing/payment truth; webhook/payment events must not auto-mutate billing-period lifecycle in first posture. Explicit deferrals remain: invoice generation, non-job invoice model expansion, billing-period invoice line items, automatic issue/send, automatic payment-link creation, Stripe checkout from billing periods, billing-period-targeted allocations, portal/self-service, autopay/subscriptions, and QBO/ACH/refunds/disputes/saved cards/partial payments/receipt automation/platform-fee execution.

**Phase 5G-B1 closeout (Billing Period Manual Invoice Link/Unlink Server Actions):**
Phase 5G-B1 is complete as server-actions only. Added wrappers `linkInternalInvoiceToBillingPeriodFromForm` and `unlinkInternalInvoiceFromBillingPeriodFromForm` in `lib/maintenance-agreements/billing-period-actions.ts`, with no UI changes in this slice. Access enforcement is active for internal Owner/Admin/Billing only via existing internal-user and financial-authority checks; dispatcher/technician/non-financial roles are denied. Link eligibility enforcement is active for required ids, same-account scope, non-cancelled and currently-unlinked billing periods, non-void and currently-unclaimed invoices, invoice customer alignment where invoice customer scope exists, and required invoice-job linkage to the same maintenance agreement through `maintenance_agreement_visits`. Unlink/correction enforcement is active for required reason and currently-linked period requirement, and remains non-destructive. State transitions are active: link sets `internal_invoice_id` and `billing_period_status = invoice_linked`; unlink clears `internal_invoice_id`, sets `billing_period_status = pending_billing`, and stores `status_reason`; both paths set `updated_by_user_id`, revalidate customer profile, and redirect with explicit success/denial/invalid/conflict banners. Runtime boundaries remain locked: no invoice generation, no invoice line-item generation, no invoice issue/send/email behavior, no payment-link creation, no payment/allocation row mutation, no Stripe behavior change, no projection/read-path switch, no `maintenance_agreement_visits` mutation, and no `next_due_date` behavior change. Validation passed for focused billing-period action tests, billing-period read-model tests, maintenance-agreements suite, `npx.cmd tsc --noEmit`, and `git diff --check`.

**Phase 6A closeout (Service Plan Automated Billing + Stripe-Saved Payment Method Audit):**
Phase 6A is complete as docs/model lock only with no implementation changes in this slice. Audit outcome: Service Plan Billing Foundation V1 remains stable for manual billing-period operations, and automated recurring billing requires additive model lanes for generated invoices, Stripe-saved payment methods, explicit autopay consent, manual saved-method charge, scheduled autopay attempts, and failed-payment retry/attention flow.
...
- `maintenance_agreement_visits` and `next_due_date` remain operational truth and do not auto-mutate from payment outcomes
...
1. 6A docs/model lock
2. 6B Generate Draft Invoice from Billing Period
3. 6C sandbox smoke for generated invoice
4. 6D saved method + consent schema/model lock
5. 6E saved method setup flow
6. 6F manual saved-method charge
7. 6G scheduled autopay attempts
...
- Boundaries preserved: no autopay enablement; no card charge attempt; no Stripe PaymentIntent money movement; no Stripe Billing Subscriptions; no `internal_invoice_payments` rows created; no `internal_invoice_payment_allocations` rows created; no invoice paid/balance mutation; no invoice issue/send/email/payment-link behavior; no `tenant_customer_autopay_consents` row created or enabled; no `tenant_saved_method_payment_attempts` row created; no `maintenance_agreements` mutation; no `maintenance_agreements.next_due_date` mutation; no customer portal behavior added
...
- Time Clock / Team Time Tracking lane is complete for current intended scope (V1 through V1.5A), including employee runtime actions, Ops status card, admin review/correction center, rolling 7-day review, and Report Center history/export.
- Timekeeping source-of-truth boundary is active and unchanged: dedicated durable internal-user time-entry truth layer; `job_events` remains narrative/timeline truth and must not become timecard truth.
- Reporting/history foundation closeout is complete in commit `3f81c71` (`feat(time-clock): add reporting history export`) with admin-only `/reports/time-clock` and CSV export.
...
- V1D-B reconciliation skips for internal/comped accounts and accounts without linked Stripe subscription, and updates only when exactly one matching `STRIPE_PRICE_ID` subscription item exists
- V1D-B reconciliation applies `proration_behavior: "none"`, while billing portal quantity editing remains deferred
...
### Tenant Customer Payment V1A-3A-2 (Stripe Connect Onboarding + Readiness Sync)

**Status**: V1A-3A-2 onboarding/readiness sync implemented with internal-admin controls on company profile.

- Added Connect onboarding helper path:
  - create connected account only when missing
  - reuse existing connected account id when present
  - create Stripe-hosted onboarding link
  - persist connected account id on `internal_business_profiles`
- Added readiness sync helper path:
  - retrieve Stripe connected account
  - update `stripe_charges_enabled`, `stripe_payouts_enabled`, `stripe_details_submitted`, `stripe_connect_onboarding_status`, `stripe_connect_disabled_reason`, `stripe_connect_last_synced_at`
  - preserve existing ready gate semantics via tenant readiness helper
- Added account-owner/admin scoped internal actions:
  - start/continue Stripe Connect onboarding
  - refresh Stripe readiness
- Added minimal internal admin company profile UI:
  - readiness state visibility
  - Connect Stripe Account / Continue Stripe Setup button
  - Refresh Stripe Status button
  - explicit not-ready and ready messaging for online invoice payments
...
### Tenant Customer Payment V1A-3E (Invoice Workspace Payment Link UI)
...
- Section 2G internal estimate chain closeout (2026-05-20): Section 2 internal estimate chain is code-complete and pushed to main/origin. Completed chain: Create estimate ? approve/select option ? convert to job ? convert to draft invoice. Local Section 2G smoke passed for both flat estimate chain and multi-option selected-option chain. Hardening commit `47f58af` (fix(estimates): harden conversion smoke edge cases) is included in main/origin. Environment-gated migrations not yet applied to production: `20260520110000_estimate_approval_response_v1.sql`, `20260520120000_estimate_to_job_conversion_v1.sql`, `20260520130000_estimate_invoice_conversion_foundation_v1.sql`. Production usability requires a controlled migration-and-smoke window. No production Supabase command was run. No production migration was applied. No env/secret changes. Still deferred: customer/public estimate approval link, customer portal approval, e-signature, stored/generated PDF artifact pipeline, automatic invoice issue/send from conversion, payment collection from estimate conversion, Stripe tenant payment execution, QBO, SMS.
...
Compliance Matters Software is a stabilized, event-driven operational system for compliance and service workflows, with complete scheduling and staffing foundations, strong auditability, completed payment/deposits reporting foundation, controlled money-flow proof still gated, and future-ready business-layer expansion.
