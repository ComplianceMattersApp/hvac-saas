...
- Customer/location truth is locked: `customers` remains account/contact/billing identity, `locations` remains saved service-address truth, jobs keep `location_id` plus copied job address snapshots, saved service addresses can be selected/created/edited, and completed job snapshots are not bulk-rewritten. See [source-of-truth-strategy.md](./source-of-truth-strategy.md).
- ECC Retest Ready parity is locked: internal users can move confirmed Retest Ready work to Needs Scheduling or Schedule Retest Now; linked retest children become the active continuation while parent/original jobs become historical/passive. Invoice/payment/cert truth remains separate. See [ECC_Guided_Workflow_Separation_Model_Lock.md](./ECC_Guided_Workflow_Separation_Model_Lock.md).
- App/portal context is locked: portal access and full-app access are separate, portal access survives inactive app access, active app + portal defaults to app, expired app + portal defaults to portal, and `/jobs/new` requires explicit portal intake context for contractor/portal submission.
- Portal equipment remains intake context only; internal users retain canonical `job_systems` / `job_equipment` authority after real job creation.
- Portal-to-app CTA is routing/visibility only and does not grant app access from portal access.
- Recent tactical UI/polish closeouts, including Account Device Setup layout, product label sweep, pending-button locks, job detail/equipment polish, calendar parity, and lifecycle response-time optimization, are recorded in [Tactical_Punch_List_Closeout_Ledger.md](./Tactical_Punch_List_Closeout_Ledger.md).
