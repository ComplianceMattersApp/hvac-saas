...
- Portal sticky navigation is locked: the `Compliance Matters Portal` sticky menu entry appears only for users with active internal app access who also have valid ECC/contractor portal access. Portal-only/non-paid users already on `/portal` do not receive a redundant sticky portal button. Valid portal access can resolve from current `contractor_users` membership or the legacy active `contractors.owner_user_id = auth user id` owner mapping; neither path grants internal authority to portal-only users or broad contractor authority to unrelated internal users.
...
**Payments, Deposits, Saved-Method, and Failed-Payment Current Truth:**
Payment current truth remains concise in this Spine; detailed payment sequencing and evidence belongs in [Compliance_Matters_Payments_Roadmap.md](./Compliance_Matters_Payments_Roadmap.md), [Financial_Ledger_Payments_Register_V1_Model_Spec.md](./Financial_Ledger_Payments_Register_V1_Model_Spec.md), [Financial_Trust_Lane_Deposits_Payout_Reconciliation_V1_Model_Spec.md](./Financial_Trust_Lane_Deposits_Payout_Reconciliation_V1_Model_Spec.md), and [Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md](./Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md).

Current source-of-truth boundaries: `internal_invoice_payments` remains collected/failed payment-event truth; `internal_invoice_payment_allocations` remains payment-to-invoice allocation truth; `tenant_saved_method_payment_attempts` remains saved-method attempt/attention truth; Stripe remains processor/payment-method/money-movement truth; invoice paid/balance remains derived from collected payment truth only. Failed payments remain non-collected and create attention/visibility only. Payment outcomes do not mutate `maintenance_agreement_visits` or `maintenance_agreements.next_due_date`.
...
**Maintenance Visit Checklist V1:**
HVAC Service-scoped maintenance visit checklist model is locked in [Maintenance_Visit_Checklist_V1_Model_Spec.md](./Maintenance_Visit_Checklist_V1_Model_Spec.md). This is a sibling spec to Checklist Foundation V1, not a successor — the two remain deliberately separate: Checklist Foundation V1 is Cleaning-scoped, high-frequency, standalone proof-of-work; Maintenance Visit Checklist V1 is Service-scoped, low-frequency, template-defined, and explicitly tied to recurring-service truth via Service Plan Templates and Maintenance Agreements. Shared field vocabulary (`is_completed`, `completed_by_user_id`, `completed_at`) is used across both specs to ease a possible future consolidation, but no shared table exists and neither implementation may assume the other exists. V1 is docs/model only and does not authorize schema, migrations, UI writes, server actions, storage behavior, or runtime checklist behavior. Phase 1 (desktop/admin: template authoring, job prefill, job detail checkbox UI, Service Plan card completion rollup) and Phase 2 (Field Mode, V2 surface only, requires separate mobile-surface audit and explicit owner sign-off before any Phase 2 spec is written) are sequenced and gated independently.
...
Service Plans command-center cleanup is closed for current pass and field-feedback gated. Current Service Plan / Maintenance Agreement truth belongs in [Maintenance_Agreements_V1_Model_Spec.md](./Maintenance_Agreements_V1_Model_Spec.md); Service Plan billing truth belongs in [Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md](./Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md).

**Financial Ledger, Allocation, and Service Plan Billing Current Truth:**
The detailed phase evidence for payment reversals, allocation dual-write/backfill, billing-period foundations, billing-period invoice linkage, and Service Plan automated-billing planning belongs in [Financial_Ledger_Payments_Register_V1_Model_Spec.md](./Financial_Ledger_Payments_Register_V1_Model_Spec.md), [Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md](./Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md), and [Maintenance_Agreements_V1_Model_Spec.md](./Maintenance_Agreements_V1_Model_Spec.md).

Current source-of-truth boundaries: Maintenance Agreement remains recurring service obligation truth; Billing Period remains commercial coverage-window truth; Internal Invoice remains billed commercial truth; Internal Invoice Payment remains collected/failed payment-event truth; Payment Allocation remains invoice-targeted allocation truth; Stripe remains processor/payment-method/money-movement truth; saved-method/autopay records remain instruction/consent/attempt truth where applicable. Billing-period paid state is derived display truth only and does not become operational truth. Payment status does not mutate `maintenance_agreement_visits` or `maintenance_agreements.next_due_date`.

Current posture: allocation and billing-period foundations are present for current intended scope, billing-period work remains non-operational/non-blocking for jobs and visits, and deeper Service Plan billing/autopay/generated invoice expansion remains governed by the payment/service-plan model specs and roadmap.

**Service Plan Billing Generation Current Posture:**
Service Plan billing-period generation, saved-method setup, and autopay consent details are governed by [Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md](./Payments_V2_Service_Plan_Billing_Foundation_Model_Spec.md). Current posture: billing periods can relate to normal job-scoped internal invoices where already implemented, while issue/send/payment, saved-card, autopay, and generated-invoice expansion remain governed by the payment/service-plan specs and roadmap. Payment outcomes still do not mutate visit count or next-due operational truth.
...
- Current-state summary: Time Clock / Team Time Tracking, Closeout Queue V1, Calendar Work Context, job detail command-center polish, internal invoice workspace/send clarity, proposal email/customer approval flow, ECC subcool tolerance, and Pricebook display-name cleanup are complete for the current intended scope.
- Evidence and tactical UI/polish details belong in [Tactical_Punch_List_Closeout_Ledger.md](./Tactical_Punch_List_Closeout_Ledger.md), [Estimates_Production_Enablement_Runbook.md](./Estimates_Production_Enablement_Runbook.md), and the relevant domain specs.
- Durable truth remains unchanged: Invoice Charges are billed truth, Work Items/Visit Scope are operational work scope, external billing remains lightweight invoice-sent tracking, payment truth is owned by `internal_invoice_payments`, Closeout Queue V1 uses existing closeout projection truth, and Calendar Work Context is derived display.
- Parked follow-ups remain roadmap-owned: Closeout Queue V2, contact recipient write/edit workflow, SMS/provider messaging, and Payments V2 deferred expansion.
...
- Manual Text Logging Wording Clarification is complete and pushed in commit `36460b8`:
  ...
  - SMS On-The-Way V1 Workflow Simplification is locked:
    ...
    - SMS Token Rendering V1 (F5C-B/F5C-C/F5C-D) is complete and merged to `main` at commit `be7fce3e` with 165 SMS communications tests passing; this closes the F5C intent-creation sequence and upgrades `sms_message_intents.message_body_snapshot` from sample-placeholder text to real rendered values.
    - New helper `lib/communications/sms-on-the-way-token-renderer.ts` adds `renderOnTheWayMessageBody` and `formatAppointmentContext`; the quiet-hours write-vs-skip policy is locked in its header comment: a blocked send must still write an `sms_message_intents` row with `decision_outcome = blocked` and `quiet_hours_decision = blocked_quiet_hours`, never skip the write.
    - `lib/communications/sms-on-the-way-intent-eligibility.ts` additively surfaces `bodyTemplate: string | null` on the result shape, populated from `selectedTemplateVersion.bodyTemplate` (raw `{{token}}` form); sample-preview behavior is unchanged.
    - `lib/communications/sms-on-the-way-intent-create.ts` adds an optional `tokenValues` param and renders the real body when token values and a body template are present on a ready decision (falls back to sample preview when absent); `DECISION_POLICY_VERSION` bumped to `f5c-b-on-the-way-intent-create-v2-real-tokens`.
    - `lib/actions/job-actions.ts` on-the-way branch resolves tech display name (`resolveUserDisplayMap`, fallback `your technician`), company name (raw null-safe `internal_business_profiles` read, fallback `our team`), and appointment context (`formatAppointmentContext` from `scheduled_date`/`window_start`/`window_end`, preferring the schedule actually applied in the update), all best-effort, then threads them as `tokenValues` into `createOnTheWayIntentFromEvent`; token-resolution failure never affects the Mark On The Way lifecycle transition.
    - Locked token fallbacks: `recipient_first_name` -> `job.customer_first_name` -> `there`; `operator_or_tech_name` -> acting user display name -> `your technician`; `company_name` -> `internal_business_profiles.display_name` (raw, null-safe; platform default `Compliance Matters` is never injected into customer-facing messages) -> `our team`; `appointment_or_job_context` -> formatted date/window -> `your service appointment`.
    - Production validation note: `sms_message_intents` remains empty until `contact_recipients` has rows for real customers; the eligibility helper correctly write-skips/no-inserts when no recipient is found (F5C-A model lock). Full production validation against real intent rows is deferred to Twilio onboarding, when `contact_recipients` is populated.
    - Remaining for full SMS activation (deferred, requires Twilio): Twilio account setup + A2P registration + sender identity configuration; `contact_recipients` population; quiet-hours implementation (write-vs-skip policy already locked); F6B preflight wiring into the Mark On The Way path; F6D webhook/callback readiness; legal/provider review gate; explicit activation approval.
    - Boundaries preserved: no schema/migration/RLS changes, no Stripe/QBO/ECC/payment behavior changes, no `insertJobEvent` signature/behavior change, no Mark On The Way lifecycle change, no Twilio/provider calls, no live SMS; `liveSendEnabled` remains false everywhere.
...
- **Group 9A-16D — Service Plan Visit Badge on Job Surfaces — CLOSED** (commit `914fe09`):
  - **Customer profile job history:** after the jobs query, a batch query fetches `maintenance_agreement_visits.job_id` for all job IDs on the customer, scoped to `link_source = 'service_plan_prefill'` and `count_status != 'reversed'`, result collected into a `Set<string>`. Teal "Service Plan" chip rendered after the ops-status chip in both case-grouped rows and ungrouped rows. Zero visual change on non-linked jobs. Query wrapped in try/catch — fail safe, badge is non-critical.
  - **Job detail desktop workbench header:** `maintenanceAgreementPromise` extended to return `planLinkContext: { agreementId: string; agreementName: string } | null` derived from the already-loaded `maintenanceLinks` array (first non-reversed link's `.agreement_id`; `agreementName` from `suggestedNextDueProjection?.agreementName` if counted link exists, else `"Service Plan"`). No second Supabase call added. Desktop workbench header renders a teal "Service Plan Visit" chip followed by the agreement name as a deep-link to `/customers/{customerId}?tab=service-plans&maFocus={agreementId}#maintenance-agreement-{agreementId}`.
  - **Ops queue (Surface 3):** deferred — `workspaceSelect` in `app/ops/page.tsx` carries no plan link field; a new join is required. Not a blocker for 9A-16D.
  - **Flag behavior:** both surfaces gated behind `ENABLE_MAINTENANCE_AGREEMENTS`; flag off → no badges, no context line, no errors.
  - **No schema changes, no migrations, no server action changes.** Read-only UI only.
  - **Validation:** `npx.cmd vitest run lib/maintenance-agreements/__tests__` passed (184/184), `npx.cmd tsc --noEmit` clean, `git diff --check` clean (pre-existing CRLF warnings only, exit 0). Browser smoke: plan-linked job shows badge, non-linked job unchanged, job detail context line visible with correct agreement name and deep-link, flag-off state shows no badges.

- **Group 9A-16B — Template Picker on Create — CLOSED** (commit `73f78ea`):
  - Replaced the inline `<details>` / GET-form template selector with `ServicePlanCreateFlow`, a new `"use client"` modal picker component at `components/maintenance-agreements/ServicePlanCreateFlow.tsx`. No new Supabase query — templates are already loaded server-side as `agreementTemplates` and passed as a prop.
  - **Picker step:** template cards render `template_name`, human-readable cadence (`CADENCE_LABELS` map matches 9A-16A), and `default_visit_scope_summary` (2-line clamp). Clicking a card closes the picker and opens the form step prefilled. "Or create without a template" text link opens a blank form.
  - **Empty state:** "No plan templates set up yet." + "Create manually" button + "Set up templates" link to `/ops/admin/service-plan-templates` (9A-16C route).
  - **Form step:** "Starting from: [Name]" note with "Start from scratch instead" link. `formKey` counter forces remount of `agreement_name` input, `MaintenanceAgreementCadenceFields`, and `VisitScopeBuilder` when template changes or scratch is selected. Hidden inputs: `customer_id`, `source_template_id`, `agreement_type`. Server action (`createMaintenanceAgreementFromForm.bind(null, customerPath)`) passed as prop — no new server action.
  - **Cleanup:** removed `maTemplate` search param, `maintenanceAgreementTemplateId`, `selectedAgreementTemplate`, `createAgreementTypeDefault`, `createAgreementFrequencyDefault`, `createAgreementVisitScopeSummaryDefault`, `createAgreementVisitScopeItemsDefault` from `app/customers/[id]/page.tsx` (all were URL-param-based prefill vars, now dead). Removed unused `MaintenanceAgreementCadenceFields` import from customer page (moved into component).
  - **Test:** `customer-template-prefill-wiring.test.ts` rewritten — checks `ServicePlanCreateFlow` wiring in customer page, component field contracts, cadence/scope fields, empty state strings, "Starting from"/"Start from scratch" strings, and absence of automation side effects.
...
- **Group 9A-16F — Template Work Items Label Fix + Compact Agreement Card — CLOSED** (commit `a884e6c`):
  - **Part 1 — Template Work Items:** Split `VisitScopeBuilder` in `TemplateCreateForm` and `TemplateEditForm` into two separate sections: (1) standalone `<textarea name="default_visit_scope_summary">` labeled "What's included (optional)"; (2) `VisitScopeBuilder` with `hideSummaryField={true}` and `summaryName="__unused_scope_summary__"` + `itemsName="default_visit_scope_items_json"`, labeled "Default work items (optional)" with helper text "These work items prefill automatically every time this template is used to create a new service plan." No server action changes — both `createServicePlanTemplateFromForm` and `updateServicePlanTemplateFromForm` already correctly persisted both fields. 2 new tests (22 total in template-actions.test.ts).
  - **Part 2 — Compact Agreement Card:** Bug fix: removed "Inactive — [date]" due-date chip for `dueState === "inactive"` (cancelled/expired plans). This chip was conflicting with the 9A-16E status badge. Now terminal plans show no due-date chip at all. Visit count summary added to compact header: "{used_visits} of {total_links} visits used" (only when `total_links > 0`). Plan Snapshot, Billing Periods, Edit Details, and `ServicePlanTerminalActions` wrapped in `<details>` element: default collapsed for active/paused/cancelled/expired; default open for `status === "draft"` or `maSaved === "created" && maFocus === agr.id` (freshly created plan shows schedule CTA). `<summary>` shows "Plan details" with webkit disclosure marker hidden.
  - **Validation:** 213/213 vitest, tsc clean, `git diff --check` exit 0. Browser smoke: manual.
  - **No schema changes, no server action changes, no migrations.**
...
Completed production-shipped ECC/test cleanup: Refrigerant Charge now uses a mode-based documentation path: Enter readings, Photo evidence, or Exception / Not applicable. Enter readings remains the numeric source for computed refrigerant-charge pass/fail. Photo evidence is a guided inline path inside the Refrigerant Charge task with separate Take Photo and Upload Photo controls; Take Photo uses camera capture behavior, while Upload Photo allows existing photo/file selection without forcing camera capture. Evidence saves as normal job attachments with refrigerant charge evidence context, preserves the original file_name, uses clean user-facing labels/titles that do not expose internal evidence tags, and appears back inside the Refrigerant Charge panel. Editing the visible attachment title/label must preserve refrigerant evidence context. Attachment Library remains the storage/history surface, not the required first stop for capture. Photo evidence mode hides numeric Refrigerant Charge entry fields and requires explicit Pass / Fail / Needs Review before completion. Photo capture/upload alone does not set computed_pass = true; Needs Review remains non-pass/non-fail; explicit Pass/Fail is user-selected outcome/override behavior, not inferred from photo evidence. Package unit, weather, and conditions exceptions remain separate from photo evidence. Equipment Label Photo evidence extends the same inline field-capture pattern to Add Equipment / System Inventory: Take Label Photo and Upload Label Photo are adjacent to the add-equipment action area, Add Equipment is withheld until manual equipment details or saved label-photo evidence exists, and cooling-equipment Tonnage stays visible outside Enter Details because airflow and related ECC defaults hinge on it. Duct leakage override suggestions now include Asbestos while preserving custom/manual reason behavior.
...
Payment P1 foundation is complete. Tenant customer invoice payments V1 is complete for the current intended scope.

Current implementation truth:
- tenant customer invoice checkout uses the approved connected-account direct-charge path with webhook-only payment truth writeback
- payment P1 foundation is closed at the current baseline
...
- Pre-launch priority ordering update:
  ...
  - This priority remains separate from tenant customer invoice payments V1, which is complete for the current intended scope.
...
---
## Service-Plan Billing Period / Payment Evidence Summary
