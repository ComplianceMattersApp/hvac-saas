...
- Next estimate direction: V1I is now recorded as planning/decision artifact only; if implementation is approved later, keep it to a small internal document-template/readiness slice first, then sandbox-only provider enablement after go/no-go gates. Do not enable production estimate email sending without an explicit rollout plan.
