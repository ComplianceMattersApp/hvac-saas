...
- Scope lock: audit/model/planning only. No implementation, migrations, scheduler jobs, Supabase mutation, Stripe calls, webhook changes, or commits.
...
- no in-flight `scheduled_autopay` for same invoice.
- respect consent `max_amount_cents` if set.
...
- manual/internal runner first with dry-run and commit modes.
- no cron in first slice.
- issued invoices only; draft excluded.
- due-today and overdue issued invoices first.
...
- `eligible` remains non-persisted dry-run/read-model output.
...
- metadata on PaymentIntent: `account_owner_user_id`, `customer_id`, `invoice_id`, `attempt_id`, `billing_period_id`, `maintenance_agreement_id`.
- only pre-webhook status transition allowed: `submitted`.
- requires_action maps to `failed_requires_action`; decline maps to `failed_declined`.
- payment rows remain webhook-created only.

Retry/failure/attention lock:
- no auto-retry loop in first 6G implementation.
- failure statuses open attention.
...
- invoice workspace: attempt history separate from recorded payments.
- customer profile: autopay consent + latest attempt outcome.
- maintenance agreement card: autopay attention summary.
- payments register: failed attempts excluded from collected totals.
- collected totals remain recorded-payment truth only.

Security/access lock:
- Owner/Admin/Billing only: consent state mutation, scheduler dry-run/commit-run, retry submit.
...
- output includes invoices evaluated, eligible count, blocked count by reason, in-flight attempts, consent/method/profile/readiness flags, proposed amount snapshots, explicit no Stripe calls/no writes.
- commit mode opt-in and separate.

Schema lock:
- no schema change needed for 6G-B dry-run eligibility model.
- no schema change strictly required for 6G-C attempt creation.
- keep `eligible` non-persisted.
- optional future additive fields only: `scheduler_run_id`, `cycle_key`, `last_blocked_reason_at`.

Risks:
- eligibility-submit race, duplicate-attempt risk, requires_action loops, cross-tenant failure cascade, missing billing-period linkage, attention UX gaps.

Recommended sequence:
1. 6G-B eligibility read model + dry-run tests.
2. 6G-C scheduled_autopay attempt creation only.
3. 6G-D submit through shared saved-card charge path.
...
Time Clock / Team Time Tracking closeout note (May 2026):
- Time Clock V1 through V1.5A is complete for current intended scope.
- Timekeeping source-of-truth remains locked to a dedicated durable internal-user time-entry layer; `job_events` may later receive summaries but must not become timecard truth.
- Completed delivery includes employee runtime actions, admin review/correction controls with required reason/audit metadata, rolling 7-day admin review, and Report Center history/export.
...
- Additive schema foundation is complete through migration `20260526150000_maintenance_agreement_billing_periods_foundation.sql`.
- New table `maintenance_agreement_billing_periods` is now the first-posture commercial coverage-window schema surface for Service Plan Billing Periods.
- Schema enforces locked lifecycle/posture values, coverage-window validity, nonnegative amount, currency format, coverage-window uniqueness, and optional internal-invoice uniqueness.
- Trigger/function scope checks enforce same-account integrity for maintenance agreement, optional customer alignment, and optional internal invoice account/customer consistency where available.
- Account-scoped RLS is enabled with SELECT/INSERT/UPDATE and no DELETE policy.
- No operational-service model mutation was introduced: jobs/work orders/visits/visit counting/next-due workflows remain available without billing-period or payment attachment.
- No UI, invoice generation, payment behavior, allocation projection/read-path switch, Stripe checkout/webhook behavior, or service-plan runtime workflow changes were introduced.
- Local migration chain validation succeeded; sandbox/production apply remains a separate approval-gated step.
...
- Added `lib/maintenance-agreements/billing-period-read-model.ts` as the read-only helper surface for service plan billing periods.
- Derived payment display states now distinguish `not_invoice_backed`, invoice draft/void, unpaid, partially paid, paid, and `payment_attention` based on current internal invoice truth and recorded payments only.
- The helper keeps payment allocation tables off the read path and does not expose forbidden operational or blocking fields.
...
- Access is enforced to active internal Owner/Admin/Billing only via existing financial authority checks.
- Manual link eligibility is now enforced for required ids, same-account scope, non-cancelled/unlinked billing period state, non-void/unclaimed invoice state, invoice customer alignment where scoped, and required invoice-job linkage to the same maintenance agreement through `maintenance_agreement_visits`.
- Manual unlink/correction behavior is now enforced for required reason and currently-linked period requirement; unlink is non-destructive and clears only relationship state.
- State transitions are now active:
  ...
  - both paths set `updated_by_user_id`, revalidate the customer profile path, and redirect with explicit query-param banners
- Runtime boundaries are preserved:
  ...
  - no payment row/allocation row mutation
...
- Real customer-profile UI generation path was used; no manual DB mutation was used.
- Fixture:
...
- Baseline verified before submit:
  - billing period unlinked (`internal_invoice_id = null`), `pending_billing`, amount `1950`, cadence `monthly`, coverage `2026-12-01` to `2026-12-31`
  - anchor active non-void invoice count `0`
  - agreement visit count `5`
  - agreement `next_due_date = 2026-09-15`
- UI smoke result:
  - `Generate Draft Invoice` control rendered for eligible period
  - anchor job id entered via UI and submit succeeded
...
- Phase 6C is now closed; next lane is Phase 6D (saved-method + consent schema/model lock).

Phase 6D-C closeout (Saved Payment Method + Autopay Consent Schema/Model Lock, docs/model only):
- Phase 6D-C is complete as docs/model lock only; no implementation, migration, Stripe API call, sandbox mutation, production touch, or webhook behavior change occurred.
...
- Saved-method model lock:
  ...
  - ACH/bank fields are future/deferred display-safe metadata only; no ACH/bank-debit behavior is activated
  - Compliance Matters stores safe references/display metadata only and never stores full card number, CVC, raw bank/card credentials, Stripe secrets, client secrets, or reusable payment credentials
- Consent lock:
...
- Attempt/payment truth boundary:
  ...
  - invoice paid state and collected money truth remain only in `internal_invoice_payments` and payment allocations after webhook confirmation
  - manual charge actions and schedulers must never directly mark invoices paid
- Event identity lock:
  - `tenant_stripe_event_receipts` is the additive event-receipt surface for setup outcomes, payment-method attach/update/detach handling, off-session attempt outcomes, duplicate handling, and connected-account-context verification
...
- Forward sequence: F6B provider delivery preflight/helper only, F6C-A manual sandbox send model lock, F6C-B server-only provider config resolver with no sends, F6C-C manual admin-only sandbox action only after explicit setup approval, F6D status callback planning/implementation before live send, then later sandbox/live SMS only after explicit approval.
...
- Future F6C-C manual admin-only sandbox send action can query for `sms_provider_deliveries` rows with `provider_status = not_submitted` to identify eligible intents for Twilio submission only after F6C-A/F6C-B gates and explicit Twilio sandbox/env/test-recipient setup approval.
...
SMS Slice F6C-A manual sandbox send model lock (May 2026):
...
- First sandbox send remains manual/admin-only and must consume an existing `sms_provider_deliveries` row with `provider_status = not_submitted`.
- Future action must not be triggered by Mark On The Way and must not live on job pages; Mark On The Way remains lifecycle + non-sending intent/preflight only.
- Required gates include admin actor, same-account delivery, `provider_name = twilio`, no provider message id, linked same-account ready On-The-Way intent, durable event anchor, required snapshots, active opted-in unsuppressed recipient, sandbox template match, sandbox provider config, verified/active sandbox sender identity, Messaging Service ref, server-only sandbox gate, and verified sandbox/test recipient limitation.
- Quiet-hours must not block lifecycle behavior; manual sandbox sends should remain verified-test-recipient-only or fail closed until quiet-hours exists.
- Future provider delivery transition reserves with guarded update from `not_submitted` to `submitted` immediately before provider handoff; if no row is reserved, no Twilio call may happen.
- On future Twilio response, Message SID maps to `provider_message_id`, raw status maps to `provider_raw_status`, and normalized initial status maps to `queued` or `submitted`; immediate provider error maps to `failed` with error fields.
- Never set `sent` or `delivered` without callback truth; never update job status; never create job timeline delivery claims.
- Current schema has no true in-flight status, so using `submitted` as reservation state is acceptable only for tightly controlled manual sandbox smoke; stronger retry/in-flight model remains parked.
- UI remains deferred; later allowed wording is `Submit sandbox SMS test`, `Sandbox submit attempted`, `Queued by provider`, or `Accepted by provider`; avoid `Send to customer`, `Delivered`, `SMS enabled`, `Live SMS`, and job-detail send buttons.
- Missing webhook is acceptable only for manual sandbox smoke; live send remains blocked until callback/signature validation, inbound opt-out or Advanced Opt-Out handling, idempotency, duplicate/out-of-order handling, payload retention, legal/provider/A2P/STOP/HELP review, and explicit activation.
...
SMS Slice F6C-C3A closeout note (May 2026):
- Slice F6C-C3A Sandbox Test Recipient + Send Gate Model Lock is complete in docs/model-only mode.
- F6C-C3A keeps real sandbox send deferred, Twilio/provider calls deferred, and live send deferred.
- F6C-C3A keeps Mark On The Way non-sending.
- F6C-C3A locks first real sandbox send to verified sandbox/test recipients only.
- Current dry-run blocker `sandbox_test_recipient_required` remains correct until verified test-recipient gate is modeled.
- Test-recipient approval must be account-scoped/admin-controlled and cannot be client-trusted.
- Preferred future test-recipient model is account-scoped `sms_sandbox_test_recipients` (or equivalent account setting) with likely fields: account scope, normalized phone or safe hash, display label, active flag, verification actor/time, timestamps.
- Test-recipient approval has no customer/job dependency, does not imply broad customer communication permission, and does not imply live-send approval.
- F6C-C3A keeps send-gate requirement fail-closed on missing/disabled state and prefers explicit account-scoped/server-only gate field on `sms_provider_configurations` (for example `sandbox_send_enabled`) or intentionally chosen account-level gate.
- F6C-C3A keeps resolver disambiguation requirement: account + `provider_name = twilio` + `provider_environment = sandbox`; production config cannot satisfy sandbox readiness.
- F6C-C3A no-go boundaries remain: no job-page send button, no Mark On The Way trigger, no live send, no SMS-enabled language, no delivered claims, no provider credentials in browser, no `NEXT_PUBLIC_*` secrets.
...
SMS Slice F6C-C3D closeout note (May 2026):
- Slice F6C-C3D Dry-Run Action Test-Recipient Gate is complete in implementation commit `e5060e9`.
- F6C-C3D added helper file: `lib/communications/sms-sandbox-test-recipient-read.ts`.
- F6C-C3D helper API: `readSmsSandboxTestRecipientForPhone({ supabase, accountOwnerUserId, phoneE164 })`.
- F6C-C3D updated action: `lib/actions/sms-sandbox-send-actions.ts` now includes test-recipient lookup gate after provider resolver gate.
- F6C-C3D updated tests: `lib/actions/__tests__/sms-sandbox-send-actions.test.ts` with 21 focused tests for all gate scenarios.
- F6C-C3D dry-run action now passes only when:
  - delivery/intent gates pass (provider_name=twilio, provider_status=not_submitted, no provider_message_id, message_class=on_the_way, decision_outcome=ready_for_provider, job_event_id present, snapshots present)
  - provider resolver passes (sandbox send gate enabled)
  - matching active verified sandbox test recipient exists (same account, matching phone, is_active=true, verified_at not null, verified_by_user_id not null)
- On success: returns `sandbox_reservation_dry_run_ready` dry-run notice (dry-run only, no send/mutation).
- On missing/inactive/unverified test recipient: blocks with `sandbox_test_recipient_required` notice.
- F6C-C3D remains evaluation-only/dry-run and does not call Twilio/provider, does not send SMS, and does not mutate `sms_provider_deliveries`.
- F6C-C3D does not set `submitted_at`, `provider_message_id`, or change provider status.
- F6C-C3D does not mutate jobs/job_events.
- F6C-C3D does not enable/trigger real SMS.
- Validation recorded: `sms-sandbox-send-actions` tests `21/21`, resolver tests `18/18`, preflight tests `17/17`, `npx.cmd tsc --noEmit` passed, and `git diff --check` passed.
- Mark On The Way still does not send SMS; real SMS remains deferred.

SMS Slice F6C-C4 closeout note (May 2026):
- Slice F6C-C4 Manual Sandbox Provider Submit Action is complete in implementation commit `98b057a`.
- F6C-C4 is the first server-only Twilio sandbox provider call path in the codebase.
- F6C-C4 added: `lib/communications/twilio-messages-client.ts` (server-only Twilio Messages REST client).
- F6C-C4 updated action: `lib/actions/sms-sandbox-send-actions.ts` with `submitSmsSandboxDeliveryToProviderFromForm(formData)`.
- F6C-C4 updated tests: `lib/actions/__tests__/sms-sandbox-send-actions.test.ts` with 26 new C4 tests (47 total).
- F6C-C4 action is admin-only, accepts only `delivery_id`, requires all gate checks (delivery, intent, resolver, sandbox gate, active verified test recipient).
...
- F6C-C4 on Twilio success: records `provider_message_id`, `provider_raw_status`, normalized provider status, and `provider_last_event_at`. Never writes `sent_at` or `delivered_at`.
- F6C-C4 on immediate Twilio error: records failed status, `failed_at`, sanitized error code/message/raw status, and `provider_last_event_at`.
- F6C-C4 does not mutate `jobs`, `job_events`, `sms_message_intents`, invoices, payments, QBO records, or portal records.
- F6C-C4 does not add webhook/status callback behavior or UI/route exposure.
- F6C-C4 does not trigger Mark On The Way behavior; Mark On The Way still does not send SMS.
- F6C-C4 Twilio credentials are read from server env vars only; never exposed in notices, logs, or FormData.
- All tests mock Twilio/provider behavior; no real Twilio calls were executed during tests.
- Validation recorded: `sms-sandbox-send-actions` tests `47/47`, resolver tests `18/18`, preflight tests `17/17`, `npx.cmd tsc --noEmit` passed, and `git diff --check` passed.
...
SMS Slice F5B closeout note (May 2026):
- Slice F5B Non-Sending On-The-Way Intent Eligibility Helper is complete in commit `9814340`.
- F5B added `lib/communications/sms-on-the-way-intent-eligibility.ts` and `lib/communications/__tests__/sms-on-the-way-intent-eligibility.test.ts`.
- F5B helper API is `evaluateOnTheWayIntentEligibility(params): Promise<OnTheWayIntentEligibilityResult>`.
- F5B is read-only and non-sending; it composes existing recipient, eligibility, template-governance, and provider-readiness helpers and adds F5B-specific job plus durable `on_my_way` event-anchor checks.
- F5B validates durable `on_my_way` anchor readiness, separates structural `blockedReasons` from deferred live-send `warnings`, returns `liveSendEnabled` false, and does not return `canSend`.
- F5B writes no `sms_message_intents` rows yet, writes no `sms_provider_deliveries` rows, does not change Mark On The Way behavior, preserves that Mark On The Way still does not send SMS, and keeps real SMS deferred.
- F5C should be planned/audited before implementation; likely next is non-sending blocked/skipped/ready `sms_message_intents` creation from eligible durable `on_my_way` anchors, while provider/Twilio send, webhook/status callback, and activation remain deferred.

SMS Slice F5C-A model lock note (May 2026):
- Slice F5C-A On-The-Way Intent Creation Model Lock is complete in docs/model-only mode.
- F5C-A writes non-sending `sms_message_intents` only, does not write `sms_provider_deliveries`, does not send SMS, and does not call provider/Twilio.
- F5C-A preferred anchor is explicit event-id handoff from successful `on_my_way` event insert; `insertJobEvent` optional returned id (or equivalent minimal helper) is recommended before job-actions integration.
- F5C-A write policy: write only when required schema fields can be populated from truth (durable event anchor, recipient truth, governed template/version/body snapshot).
- F5C-A no-write policy: if required fields are missing, return no-insert/write-skipped with no fake recipient/template/version/snapshot values.
...
- F5C-B added helper `lib/communications/sms-on-the-way-intent-create.ts` and tests `lib/communications/__tests__/sms-on-the-way-intent-create.test.ts`.
- Helper API: `createOnTheWayIntentFromEvent(params): Promise<CreateOnTheWayIntentFromEventResult>`.
- F5C-B creates non-sending `sms_message_intents` rows only; it does not create `sms_provider_deliveries`, does not send SMS, and does not call provider/Twilio.
- F5C-B ready mapping: writes one intent row with `decision_outcome = ready_for_provider` and `blocked_reason_codes = []` when all required truth exists.
- F5C-B blocked mapping: writes one blocked intent with `decision_outcome = blocked` only when required recipient/template/body truth exists; otherwise no-insert with `writeSkippedReason`.
- F5C-B skipped/write-skipped mapping: non-target events and missing required fields return no-insert/no-op with optional `writeSkippedReason`; no fake data is inserted.
- F5C-B idempotency: uses `${accountOwnerUserId}:${jobEventId}:on_the_way:${contactRecipientId}`; unique conflict is treated as deduped success.
...
Section 2K validation/safety facts are recorded: provider-backed email send smoke passed, public link approval smoke passed, internal notification smoke passed, active-link regeneration fallback smoke passed, Customer Delivery deployed UI smoke passed, local preview-mode smoke passed, and token safety is verified (raw token/token_hash are not persisted in events, communications, or notification payloads).
Section 2K remaining items are parked as polish only: public proposal Service Location/card wording and presentation audit, optional email visual polish using preview mode only, visual-only Customer Delivery tweaks unless field workflow evidence reopens behavior, optional notification routing refinements, and optional multi-option customer card presentation polish.
...
V1J does not add persistent revision storage.
 V1J did not add real outbound production estimate email, PDF generation/storage, persistent revision storage, customer approval/e-signature, customer portal estimate visibility, public estimate links/tokens, contractor visibility/authority, estimate-to-job conversion, estimate-to-invoice conversion, payment/deposit, Stripe tenant payment behavior, QBO behavior, or any customer-facing production estimate enablement.
 Estimate Internal Quote Readiness Checklist V1 is complete on the guarded internal baseline: estimate detail now shows a read-only internal readiness checklist using existing estimate/customer/location/line/total data plus recipient email-on-file readiness, while preserving manual lifecycle controls and all deferred customer-facing boundaries.
...
- **Payments Register V1C is now implemented and browser smoke passed** (commit `55dab8c`): read-only customer profile Payment History on `/customers/{id}` with Owner/Admin/Billing gating, default block for Dispatcher/Technician/Contractor/Portal users, recorded/failed/other status separation, and Open Payments Register deep-link to filtered `/reports/payments`. Customer profile history reads from `internal_invoice_payments` current truth only and keeps ACH hidden/mapped to `other`.
- V1C deferred remains explicit: register-based payment recording/corrections, payment allocations, broader financial dashboard cards, QBO, ACH, platform fees, recurring billing, and customer portal payment history.
