...
| 9B | SMS / On-My-Way Messaging | Planned (staged) — Slice A Recipient Registry Foundation is complete (`afddb9c`) with final migration `supabase/migrations/20260515120000_contact_recipients_slice_a_foundation.sql`; timestamp hygiene rename closeout is complete (`02aee5a`) from original `20260514120000_contact_recipients_slice_a_foundation.sql` due to existing timestamp use by `20260514120000_maintenance_agreement_visits_next_due_confirmation_metadata.sql`; Slice B1 Consent + Suppression Foundation is complete (`39a2963`) with migration `supabase/migrations/20260515123000_contact_recipient_consent_suppression_foundation.sql` creating `contact_recipient_consents` and `contact_recipient_suppressions` (consent default `unknown`, fail-closed on missing/unknown consent, active suppression precedence over consent). Slice B2 Non-Sending Eligibility Inputs Helper is complete (`c0247af`) adding `lib/communications/sms-eligibility-inputs-read.ts` plus tests in `lib/communications/__tests__/sms-eligibility-inputs-read.test.ts`; helper reads only recipient/consent/suppression sources, does not read jobs/customers/locations/job_events, returns non-sending input state only (`nonSendingStatus`, `blockedReasons`), does not return `canSend`, and keeps suppression-first blocked ordering. Slice B2 validation passed: helper tests `16/16`, Slice A helper tests `4/4`, `npx.cmd tsc --noEmit`, and `git diff --check`. Slice C docs/model closeout is complete with `docs/ACTIVE/SMS_Background_On_The_Way_Workflow_Spec.md`: no job-detail preview card for V1, no field free-text editor, admin-only future template governance, and future background/event-driven On-The-Way evaluation after lifecycle transition with explicit gate/failure contracts. Slice D docs/model closeout is complete with `docs/ACTIVE/SMS_Settings_Communications_IA_Spec.md`: Settings -> Communications is the future home for messaging governance; Company Profile boundary is preserved for business identity/support contacts; future Communications section IA and activation-control posture are documented. Slice E1 docs/model closeout is complete with `docs/ACTIVE/SMS_Message_Intent_and_Provider_Delivery_Model_Spec.md`: V1 intent/delivery cardinality, idempotency, provider-neutral status normalization, callback write path, and RLS expectations are locked and a combined migration foundation is the likely next implementation. No remote/sandbox/production migration apply and no production writes. Scope remains no SMS intent/provider delivery tables, no send endpoint/webhook/provider integration, no live SMS, and no backfill. Live SMS remains deferred pending activation gates in `docs/ACTIVE/SMS_Compliance_and_Consent_Model_Spec.md`, recipient-role boundaries in `docs/ACTIVE/SMS_Recipient_and_Contact_Role_Model_Spec.md`, and future slices documented in `docs/ACTIVE/SMS_Recipient_Consent_Schema_Design_Plan.md`, `docs/ACTIVE/SMS_Background_On_The_Way_Workflow_Spec.md`, `docs/ACTIVE/SMS_Settings_Communications_IA_Spec.md`, and `docs/ACTIVE/SMS_Message_Intent_and_Provider_Delivery_Model_Spec.md`. |
