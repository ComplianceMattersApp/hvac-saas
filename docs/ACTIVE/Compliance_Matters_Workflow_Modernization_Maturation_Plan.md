...
Equipment Label Photo evidence now follows the same field-first capture principle inside Add Equipment / System Inventory. Field users can Take Label Photo or Upload Label Photo directly beside the add-equipment actions, and Add Equipment appears only after manual equipment details or saved label-photo evidence exists. For cooling equipment, Tonnage remains visible outside Enter Details because downstream airflow and related ECC defaults hinge on that value even when the equipment record is primarily supported by a label photo. Enter Details keeps secondary fields such as manufacturer, model, serial, refrigerant, and notes available without making them the first step.
