...
- Roadmap posture: read-only payments/deposits reporting is closed for current intended scope; detailed model/evidence belongs in [Financial_Ledger_Payments_Register_V1_Model_Spec.md](./Financial_Ledger_Payments_Register_V1_Model_Spec.md) and [Financial_Trust_Lane_Deposits_Payout_Reconciliation_V1_Model_Spec.md](./Financial_Trust_Lane_Deposits_Payout_Reconciliation_V1_Model_Spec.md).
- Future gate remains controlled production money-flow proof: one real/live paid invoice or existing paid production invoice, one scoped settlement sync, Stripe Dashboard gross/fee/net comparison, Deposits report/detail/CSV verification, and payout/bank deposit confirmation when available.
...
| Estimates follow-on expansion | 3 | Signed customer proposal delivery/approval, explicit job/invoice-draft conversion, two-or-three-option comparison proposals, Estimate AI/Trainer, and estimate photos are implemented. Payment/deposit collection, e-signature/legal artifacts, customer portal dependency, and automated conversion remain outside this lane. | Reopen only for field evidence or an explicit owner-approved expansion. |
...
1. Estimates production enablement
   ...
   - Estimate comparison proposals are implemented using one parent Estimate / Proposal with child Option Packages. New comparisons start with two populated choices; Best is an optional third.
   - July 20, 2026 closeout: Estimate Coach/line rewrite, governed Trainer/help-gap logging, owner-only $25 AI budget tracking, estimate photos with explicit customer visibility, and combined Finalize & Send Proposal are on `main`. The owner confirmed `20260720150000_estimate_photos.sql` applied in production; owner-controlled production testing continues.
...
4. Pull next lane by operational signal, not by assumption.
   ...
   - Keep Support V1 read-only console, estimate payment/e-signature/portal expansion, Service Plans V2 improvements, and other deferred lanes parked unless owner evidence explicitly reorders them.
