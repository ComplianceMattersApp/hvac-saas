...
- Time Clock / Team Time Tracking lane is now explicitly staged as post-lock sequence work beginning with V1A docs/model lock only (no runtime behavior change in this pass).
- Time Clock sequence is locked as: V1A model/docs lock -> V1B schema/settings foundation -> V1C employee `/time-clock` -> V1D Ops/Home team-status card -> V1E admin review/correction center -> V1.5 weekly summary/export.
- Source-of-truth boundary is explicit: dedicated timekeeping table owns timecard truth; `job_events` remains narrative/timeline truth.
