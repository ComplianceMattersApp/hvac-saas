...
| 3 | Provider-powered customer SMS and two-way messaging | Housecall Pro, Jobber, and Workiz all emphasize customer communication, reminders, on-my-way texts, and messaging. | **NO LONGER A BUILD GAP (2026-08-09).** The Twilio A2P campaign was approved 2026-08-05 and a production smoke passed (delivery in ~2.2s, STOP verified). Sending, inbound/opt-out processing and the live-activation lane are implemented (`lib/communications/sms-live-send.ts`, `twilio-inbound-processor.ts`). What remains is an owner action — lift the test-phone suppression and press Activate. Two-way *conversational* messaging is still genuinely absent. |
...
| 5 | GPS, route optimization, and location timers | Jobber and Housecall Pro advertise map/routing/GPS; Workiz advertises dispatch and scheduling efficiency. | **MOSTLY BUILT (2026-08-09).** Route planning shipped 2026-08-08: `lib/routing/` does area clustering, day-fit scoring, drive-order sequencing and arrival windows, surfaced at `/calendar?view=plan`. Time clock shipped separately (`lib/time-clock/`). Still absent: **GPS** specifically — no location capture on clock events — and live drive times (straight-line estimates today; Routes API is the next slice). |
...
| 7 | QBO/accounting sync | All three competitor ecosystems surface accounting/QuickBooks integration. | **NO LONGER A GAP — arguably now a strength (2026-08-09).** Intuit production approval was granted 2026-08-03 and QBO is live in production, syncing customers, invoices, payments and (since 2026-08-09) invoice voids. A scheduled three-way reconciliation compares EveryStep against QuickBooks *and* Stripe independently and reports disagreements. Most FSM tools push to QuickBooks; few verify that the push landed or detect drift afterwards. EveryStep remains source of truth; QBO stays downstream. |
| 8 | Payments V2 add-ons | Competitors support richer payment expectations: saved cards, automatic charges, deposits, instant payouts, financing, payment reminders, and customer self-service payment experiences. | **PARTIALLY CLOSED (2026-08-09).** Shipped since this audit: saved cards and saved-method self-service, scheduled autopay with failed-autopay retry, and inbound refund/dispute handling (full refund reverses, partial is raised for manual allocation, dispute reverses only when lost). Still deferred: ACH, deposits, financing, instant payouts, public payment-portal expansion, and operator-initiated refunds from inside EveryStep. |
