...
- **Product mode capture planning (future implementation):** First owner provisioning will eventually require `--product-mode hvac_service|ecc_hers|hybrid|cleaning_services` flag. Product mode will be written to `account_settings` during apply, after owner identity resolution and before invite send. Missing/invalid product_mode will block provisioning apply once implemented. See `docs/ACTIVE/Product_Mode_Signup_Spec.md` §6.5 and `docs/ACTIVE/First_Owner_Provisioning_Runbook.md` §11 for phase 1/2/3 planning. Phase 1 does not require backfilling existing accounts. Production account_settings migration must be applied before production provisioning writes product_mode.
...
- Confirmed Product Mode V2 Slice 1 facts:
  ...
  - `/jobs/new` mapping remains `hybrid` -> ECC default, `ecc_hers` -> ECC default, `hvac_service` -> Service default, `cleaning_services` -> Service default
