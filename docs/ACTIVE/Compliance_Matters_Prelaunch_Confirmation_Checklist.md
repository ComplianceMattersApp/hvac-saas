...
- **Product Mode V2 production migration readiness closeout (planning only):** verdict is **ready after listed inputs**. Future production scope is only `supabase/migrations/20260509120000_account_settings_product_mode_v1.sql`; production remains untouched in this pass. Future preflight must include branch/worktree/doc commit/ref verification, migration list + dependency checks (`public.set_updated_at`, `public.current_internal_account_owner_id`), dry-run review, rollback owner, approver/decision channel, change window, and evidence location. No backfill/owner-hybrid write/Angkor provisioning in same migration window.
...
- **Estimate Communications V1H production migration-window addendum (planning only):** next target is only `20260502120000_estimate_communications_v1h.sql`; Product Mode `20260509120000_account_settings_product_mode_v1.sql` must remain excluded. Execution must use isolated artifact/worktree include list (`20260501120000`, `20260501140000`, `20260502120000`) and exclude list (`20260509120000`), with preflight confirmation of main branch, clean tree, docs committed/pushed, production ref `ornrnvxtwwtulohqwxop`, V1A applied, V1H pending, Product Mode pending, and flags false/unset. Sequence remains dry-run first, hard approval stop, apply only after explicit owner approval. Required post-apply checks: `public.estimate_communications` table/columns/constraints/FKs/checks/indexes/policies/RLS, row count `0`, migration history confirms `20260502120000` applied and `20260509120000` unapplied. Disabled-state smoke remains required with no exposure/no email side effects.
