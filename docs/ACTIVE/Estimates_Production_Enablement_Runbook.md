...
- Production estimate migration state for Section 2 internal chain is complete in production (`ornrnvxtwwtulohqwxop`):
...
- Estimate Multi-Option Proposal Model is documented in `docs/ACTIVE/Estimate_Multi_Option_Proposal_Model_Spec.md`: implemented comparisons use one parent Estimate / Proposal with child Option Packages; new proposals start with Good and Better and permit optional Best. Fake option headers and separate linked estimates remain rejected.
...
- Section 2C Estimate -> Job Conversion V1 closeout is implemented in commit `95c87df` as Action A only (approved estimate -> internal job/work order): flat estimates convert all flat estimate lines; multi-option estimates convert only selected-option lines. Durable linkage fields are implemented as `estimates.converted_job_id`, `estimates.converted_by_user_id`, and `jobs.origin_estimate_id` via migration `20260520120000_estimate_to_job_conversion_v1.sql`. Conversion is schema-gated by environment and is hidden/fails safely (`estimate_conversion_schema_unavailable`) until that migration exists in the target environment. This slice adds no invoice conversion, no invoice draft creation, and no payment/QBO/SMS/email/portal/provider behavior. No production Supabase command was run in this implementation slice. Production usability requires approved migration application in production later.
...
- Section 2E Estimate → Invoice Draft Conversion Schema Foundation closeout (2026-05-20) is implementation-complete for schema and deploy-safety compatibility only: migration `20260520130000_estimate_invoice_conversion_foundation_v1.sql` adds `estimates.converted_invoice_id`, `internal_invoices.source_estimate_id`, and partial unique indexes for idempotency; read-model compatibility adds `getEstimateConvertedInvoiceId()`, `isEstimateToInvoiceConversionSchemaReady()`, and readiness flags to `EstimateReadResult` and `EstimateListItem`; no invoice conversion action, no invoice draft creation, no invoice line items, no Convert to Invoice button, no issue/send/payment/QBO/SMS/email/provider/portal behavior; migration activation remains environment-gated; no production Supabase command was run in this slice.
- Section 2F Estimate → Invoice Draft Conversion V1 closeout (2026-05-20) is implementation-complete for internal Action B only: requires converted job linkage from Section 2C, requires invoice conversion schema readiness, creates draft internal invoice only (`status=draft`, `source_type=estimate`, `source_estimate_id`, `job_id`), sources invoice line items from converted job `visit_scope_items` only with frozen visit-scope provenance snapshots, updates `estimates.converted_invoice_id`, and writes `estimate_converted_to_invoice` audit event metadata; no issue/send/payment/QBO/SMS/email/provider/portal behavior was added; no production Supabase command was run; migration activation remains environment-gated.
...
- Section 2H CMTest Smoke Final Docs Closeout (2026-05-20): Section 2H CMTest manual-assisted smoke passed. Target environment: CMTest (`kvpesjdukqwwlgpkzfjm`). Runtime-origin verification passed: localhost app bundle was CMTest-bound, production markers were absent, `ENABLE_ESTIMATES=true`, and `ENABLE_ESTIMATE_EMAIL_SEND` was absent. Required Section 2 migrations were present in CMTest: `20260520110000_estimate_approval_response_v1.sql`, `20260520120000_estimate_to_job_conversion_v1.sql`, and `20260520130000_estimate_invoice_conversion_foundation_v1.sql`. Required schema columns/indexes were verified in CMTest. Flat estimate chain passed: estimate `229af34a-607d-44b5-a6eb-2ddff2546e27`, invoice `INV-20260521-E10FBF6E`. Multi-option chain passed: job `918762df-3ed5-4e66-b38a-f82f304950dd`, invoice `INV-20260521-BD772A73`, selected option `Option 3 - Best`. Idempotency/negative checks passed. Validation passed: estimate suite, invoice provenance test, TypeScript, and `git diff --check`. No code/docs changes were required during smoke. No production Supabase command was run. No production migration was applied. No env/secret changes. No invoice issue/send/payment/QBO/SMS/email/provider/portal behavior was exercised. Final status: Section 2 internal estimate chain is complete and verified in CMTest. Production remains pending a separate controlled production migration-and-smoke window.
- Section 2I Production Smoke Final Docs Closeout (2026-05-21): production target `ornrnvxtwwtulohqwxop` passed approved migration apply plus read-only schema/index verification for Section 2. Approved production migrations applied: `20260519110000_estimate_option_packages_foundation.sql`, `20260520110000_estimate_approval_response_v1.sql`, `20260520120000_estimate_to_job_conversion_v1.sql`, and `20260520130000_estimate_invoice_conversion_foundation_v1.sql`. Flat production smoke passed with estimate `EST-20260521-0DEEAAB2` (`2843a28f-12f8-4757-ba1b-3a9c964de96e`), job `123ddddd-9305-47fb-a000-e81977ef343a`, and invoice `INV-20260521-6B51BC7E`. Multi-option production smoke passed with estimate `EST-20260521-4FF043F3` (`2d725dc4-ff66-46c5-be69-a0d262c83ebd`), selected option `Better`, job `88ade054-3543-4685-a9d2-81da637cda8f`, and invoice `INV-20260521-D6518867`. Idempotency/negative checks passed. Validation passed: estimate suite `254/254`, invoice provenance tests `3/3`, TypeScript clean, `git diff --check` clean, and working tree clean/synced. No code changes were made during smoke execution. No invoice issue/send occurred. No payment collection occurred. No Stripe tenant payment execution occurred. No QBO/SMS/email/provider/portal behavior occurred. Watch item only: dev-mode SSR hydration/webpack invoice-page error recovered to client rendering and did not block data/workflow. Final status: Section 2 Estimates internal chain is complete and production-smoke passed. Remaining estimate work is future UX/polish/customer-facing scope and is not an internal-chain blocker.
- Section 2K — Estimate Proposal Email Delivery / Customer Approval Flow docs closeout is complete: provider-backed proposal email send smoke passed; public proposal-link approval smoke passed; internal approval notification smoke passed; active-link regeneration fallback smoke passed; Customer Delivery deployed UI smoke passed; local preview-mode smoke passed with rendered output written to `.tmp/email-outbox` and no real email send. Token safety is verified (`raw token` and `token_hash` are not persisted in events, communications, or notification payloads).
- Section 2K final workflow is locked as Build proposal -> Finalize Proposal -> Customer Delivery -> Send Proposal Email / Copy Link -> Customer Approves -> Internal Notification -> Review/Convert when ready.
- Section 2K explicit non-actions are confirmed: no SMS/text proposal delivery, no payment collection from proposal, no QBO behavior, no invoice issue/send behavior from proposal approval, no automatic job conversion, no automatic invoice conversion, no customer portal login dependency, no e-signature/legal artifact model, and no live SMS behavior.
- Remaining Section 2K follow-up is parked as polish only: public proposal Service Location wording/presentation audit, optional proposal email visual polish in local preview mode only, visual-only Customer Delivery tweaks unless workflow evidence reopens behavior, optional approval-notification routing enhancements, optional multi-option customer card presentation polish, and future public proposal Service Location/card wording polish.
...
| v1.9 | July 20, 2026 | Estimates AI/proposal closeout | Recorded Estimate Coach and field-friendly line rewrite support, owner-only global AI usage tracking with a $25 monthly application cap, repository-backed Trainer expansion with durable help-gap logging, estimate photo attachments and customer-visibility controls, combined Finalize & Send Proposal delivery, and two-or-three-option comparison proposals. Owner confirmed `20260720150000_estimate_photos.sql` applied in production. Code is on `main`; owner-controlled production testing continues. |
