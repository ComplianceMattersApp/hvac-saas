...
- F6C-C3A sandbox test-recipient + send gate model lock is complete in docs/model-only mode with sandbox/live send still deferred.
- F6C-C3A locks verified test-recipient gate as account-scoped/admin-controlled only; client cannot self-approve test recipients.
- F6C-C3A prefers account-scoped `sms_sandbox_test_recipients` model (or equivalent account setting) with normalized phone or safe hash, active flag, display label, verification actor/time, and timestamps; no customer/job dependency; no live-send approval implied.
- F6C-C3A locks send-gate model as account-scoped/server-only/admin-controlled, preferring explicit `sms_provider_configurations` field (for example `sandbox_send_enabled`) or intentionally chosen account-level gate.
