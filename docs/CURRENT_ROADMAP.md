...
- **Next safe slice:** LANE COMPLETE to live-capable (Aug 2026). A2P campaign approved; sandbox smoke passed in prod (submit → delivered 2.2s, STOP suppression verified). Live activation shipped: quiet-hours gate (8am–9pm account time zone), attested activate/deactivate flow in Admin Communications, auto-send on Mark On The Way when active, suppression lift UI, and one-step consent capture in the add-contact forms. Remaining niceties: appointment reminders + invoice-sent notifications (new message classes reusing this pipeline), per-recipient time zone, review-request second campaign (parked until filed).
...
- **Migrations applied to production July 12, 2026:** `20260712120000`–`20260712170000` (the six workshare migrations) are live in prod (applied alongside the two previously-pending QBO migrations `20260710120000`/`120100`, which db push carried along; QBO schema is additive and its feature stays gated on Intuit approval).
...
### QuickBooks Online Integration (Lane 5) — BUILT, optional and account-configured
- **Status:** Built and merged to `main`; available when the account owner has configured and authorized the integration. `lib/qbo/` (api client, connection, encryption, env, OAuth client, invoice sync, payment sync + tests), the OAuth callback at `app/api/qbo/callback/route.ts`, `qbo-connection-actions.ts` / `qbo-sync-actions.ts`, and the `QboIntegrationSection` in Company Profile are on `main`. Both migrations — `20260710120000` (connections foundation) and `20260710120100` (internal-invoice sync columns) — are applied to production.
- **Scope:** one-way EveryStep-to-QBO synchronization of eligible customers/invoice context, invoices and line items, and recorded payments. After Stripe or an authorized internal workflow confirms payment and EveryStep records payment truth, EveryStep attempts to create and apply the related QBO Payment to the matched QBO invoice. `intuit-oauth` only (node-quickbooks rejected).
- **Next safe slice:** harden QBO Payment creation against the provider-success/local-persistence-failure duplicate window, then validate retry recovery in the QBO sandbox before any production rollout.
- **Guardrails:** Stripe remains processor truth; EveryStep remains operational source of truth for all job, customer, invoice, payment, and closeout data; QBO remains optional downstream accounting state. Synchronization is best effort and depends on connection, authorization, provider/API availability, and record matching. Intuit secrets stay server-only; the callback runs through the `proxy.ts` bypass; `realmId` is read from the callback URL, not the token body.
...
7. **Lane 7 — SMS Self-Serve Tenant Provisioning (automated)** — **COMMITTED, not started; demand-triggered.** Owner decision (Aug 2026): automated tenant SMS onboarding is on the roadmap for certain. An "Enable texting" wizard automates per-tenant number purchase, Messaging Service creation + webhook wiring, and TrustHub brand/campaign registration, with tenant-facing registration status and rejection/retry handling. The app-side pipeline is already tenant-generic and `sms_sender_identities` already reserves the provider ref columns. Concierge onboarding (checklist in the owner doc) is the plan of record until roughly the fifth tenant requests SMS or onboarding SLA matters for GTM; start prerequisites are Twilio ISV posture review, TrustHub API access, and the SMS add-on pricing decision. Owner doc: `docs/ACTIVE/SMS_Tenant_Self_Serve_Provisioning_Lane_Spec.md`.
