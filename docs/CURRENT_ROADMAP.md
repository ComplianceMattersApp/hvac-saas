...
- **Next safe slice:** after A2P campaign approval — set `TWILIO_*` env vars, complete provider setup in the admin UI, run the controlled sandbox smoke to a verified test recipient. Then: quiet-hours gate, production provider configuration + explicit activation flow, and legal/provider review before any live send. Customer Communication Polish V1 (appointment reminders, On-the-Way, invoice-sent notifications) folds into this lane as the first high-value events when SMS activates.
