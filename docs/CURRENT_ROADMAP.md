...
- **The full loop is now built and merged (July 12, 2026):**
  ...
  - **Returned-work surfacing** — a sender-side **"Returned Work" chip** on `/ops` + queue at `/ops/workshare/returned` (persist-until-handled: a pass clears on "Mark handled", a fail on retest), so a returned result can't fall through when the source job is off-board. The legacy `/ops` handoff rail + `/ops/handoffs`/`/ops/connected-handoffs` routes were retired (routes-only; the handoff subsystem is a superseded pre-workshare path, backing left dormant for a later classic-retirement sweep).
- **Remaining before Closed:** apply migrations `20260712120000`–`20260712170000` (six) to **production** in order (deliberate, environment-verified per §15), then owner prod smoke of the full loop.
