...
### QuickBooks Online Integration (Lane 5) — BUILT, gated on Intuit production approval
- **Status:** Built and merged to `main`; **feature-gated on Intuit production OAuth approval + owner enablement.** The lane is not "unstarted" — it is code-complete and prod-migrated, held behind an approval gate rather than an implementation gap. `lib/qbo/` (api client, connection, encryption, env, oauth client, sync + tests), the OAuth callback at `app/api/qbo/callback/route.ts`, `qbo-connection-actions.ts` / `qbo-sync-actions.ts`, and the `QboIntegrationSection` in the Company Profile integrations section are all on `main`. Both migrations — `20260710120000` (connections foundation) and `20260710120100` (internal-invoice sync columns) — are **applied to production** (additive schema, carried along by the July 12 db push).
- **Scope:** two-way sync (customers, invoices, line items, payments into QBO on job completion). `intuit-oauth` only (node-quickbooks rejected).
- **Next safe slice:** none pre-approval — the remaining work is the Intuit production OAuth review/approval and owner enablement, not code. Post-approval: controlled connect + first-sync smoke.
- **Guardrails:** QBO is downstream accounting only; EveryStep remains operational source of truth for all job, customer, invoice, payment, and closeout data. Intuit secrets stay server-only; the callback runs through the `proxy.ts` bypass (Next 16 renamed `middleware`→`proxy.ts`); `realmId` is read from the callback URL, not the token body. No production sync enablement before Intuit approval.
...
5. **Lane 5 — QuickBooks Online Integration** — **BUILT; gated on Intuit production approval + owner enablement.** Full `lib/qbo/` module, OAuth callback (`app/api/qbo/callback/route.ts`), connection/sync server actions, and the Company Profile integrations section are on `main`; both migrations (`20260710120000` connections foundation + `20260710120100` internal-invoice sync columns) are applied to production (additive schema). Two-way sync (customers, invoices, line items, payments into QBO on job completion). Locked boundary: QBO is downstream accounting only; EveryStep remains operational source of truth for all job, customer, invoice, payment, and closeout data. Owner-confirmed buyer expectation and personal-use need. Feature stays gated until Intuit production OAuth approval.
