...
- ~~**Route Builder / Schedule Assist** — nice-to-have, not operationally urgent.~~ **BUILT — no longer deferred (corrected 2026-08-09).** Route planning shipped 2026-08-08: `lib/routing/` (geocoding, geometry, area clustering, day-fit, route plan engine, route links) with test coverage, a plan view at `/calendar?view=plan`, and migrations backfilled on both databases. It still has **no dedicated documentation** — treat `lib/routing/` and its tests as the authority until a spec exists.
