...
- State is stored as text and remains a visible editable field. Nationwide rollout removed the `CA` creation default from wired V1 service-location surfaces. The parser prefers `administrative_area_level_1.shortText`, and consuming forms accept the supplied U.S. state without rejection or relabeling.
...
The owner superseded the earlier California-default convenience posture: EveryStep is opening nationwide. Physical service-address State remains visible, editable, required where it was already required, and submitted under the existing `state` field name. New service locations begin with a blank state; Google selection may populate any U.S. state. The application does not reject, silently relabel, or otherwise special-case non-California selections. Existing saved locations continue to render their stored state.

The real-provider `/jobs/new` smoke was owner-performed with the restricted local browser key. It confirmed that the assistant mounts in internal new-customer/new-location mode, Google suggestions appear, and a South Carolina selection fills street, city, state, and ZIP+4. The owner also confirmed the promoted search panel and manual-entry fallback visually. Codex verified without exposing the key that the live client chunk contains the configured value, the stable script element exists, `google.maps.importLibrary("places")` succeeds, `PlaceAutocompleteElement` is available and registered, and the loader uses the direct `NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_API_KEY` reference. A transient first-import race was fixed with a bounded retry and failed-result reset. Selection now follows Google's current `gmp-select` and `placePrediction.toPlace()` contract and requests only `addressComponents`.

Automated coverage confirms missing-key safety, no DOM insertion without a key, single-flight loading, transient import retry, retry after failure, parser behavior, ZIP+4, Address Line 2 preservation, partial-result preservation, nationwide state application, route visibility, editable canonical field names, unchanged actions, and absence of submit/identity mutation from the selection handler. No Place ID, coordinates, prediction, raw components, or other provider object is persisted.

Final nationwide regression validation passed 12 files and 100 tests covering the loader/parser/shared fields, `/jobs/new`, customer create/Add Location/edit wiring, customer and location actions, duplicate-location behavior, intake ownership, contractor shared-route behavior, and invoice address rendering. TypeScript, focused changed-file ESLint, and whitespace validation passed. The broader `job-service-location-change.test.ts` run passed 8 of 9 tests; its one source-string guardrail now finds an unrelated `service_cases` update elsewhere in the large shared `job-actions.ts`. Both the test and action are unchanged from the Slice D tip and have no feature-branch diff, so this is recorded as a pre-existing stale assertion and was not modified.
