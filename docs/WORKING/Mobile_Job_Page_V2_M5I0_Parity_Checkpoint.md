...
| Work Scope / Visit Scope | Native parity accepted after consolidation | Medium/High | `MobileJobWorkScopePanel` renders current, `v2TargetPanel`, and `v2DisclosurePanel`. V2 embeds details into the Work to Do / Work Performed card and preserves `updateJobVisitScopeFromForm`, `VisitScopeJobDetailForm`, `mobile-work-scope`, and `mobile-visit-reason-card`. | Field-confirm Add/Adjust Work remains stable; do not touch during unrelated slices. |
...
- Work Scope duplicate-card/disclosure regressions were corrected; Work Scope details now live with the Work to Do / Work Performed area.
