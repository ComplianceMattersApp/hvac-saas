        ...
        <ul className="mt-3 space-y-2">
          {coverage.assignees.map((row) => (
            <li key={row.key} className={ROW_SHELL}>
              <div className={wide ? "grid gap-1.5 sm:grid-cols-[minmax(8rem,0.75fr)_minmax(0,1.6fr)_auto_auto] sm:items-center" : "flex items-center justify-between gap-3"}>
                <div className={wide ? "min-w-0" : "grid gap-1.5"}>
                  <div className="text-sm font-semibold text-[#0f1f35]">{row.assigneeName}</div>
                  {!wide && (
                    <div className="truncate text-sm leading-5 text-slate-600" title={row.areaLabel}>
                      {row.areaLabel}
                    </div>
                  )}
                </div>
                {wide && (
                  <div className="min-w-0 truncate text-sm leading-5 text-slate-600" title={row.areaLabel}>
                    {row.areaLabel}
                  </div>
                )}
