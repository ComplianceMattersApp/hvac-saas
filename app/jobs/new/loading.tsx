export default function NewJobLoading() {
  return (
    <div className="space-y-5 px-3 py-4 sm:px-6" aria-busy="true" aria-live="polite">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="space-y-2">
              ...
              <div className="h-11 animate-pulse rounded-lg border border-slate-100 bg-slate-50" />
            </div>
          ))}
        </div>

        <div className="mt-5 h-11 w-36 animate-pulse rounded-lg bg-blue-100" />
      </div>
    </div>
  );
}
