          ...
          >
            {pulseOperationalCards.map(([title, eyebrow, body, action, iconName], index) => (
              title === "Job Brief" && pulseJobBriefCardContent ? (
                ...
                <div key={title}>{pulseWorkToPerformCardContent}</div>
              ) : (
                <div
                  key={title}
...
const failureResolutionPathCount =
  ...
  const pulseWorkToPerformCardContent = (
    <div
      data-v2-zone="pulse-work-to-perform-card"
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
            ...
            <span>Work to Perform</span>
          </div>
          <h3 className="mt-1 text-sm font-semibold text-slate-950">{pulseWorkToPerformModel.title}</h3>
        </div>
      ...
      {pulseWorkToPerformModel.mode === "items" ? (
        <div className="mt-2 space-y-1.5">
          <div className="line-clamp-2 text-xs leading-5 text-slate-600">{pulseWorkToPerformModel.body}</div>
          <div className="space-y-1.5">
            {pulseWorkToPerformModel.previewItems.map((item, index) => (
              <div key={`${index}-${item.title}`} className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2">
                <div className="text-xs font-semibold leading-5 text-slate-900">{item.title}</div>
                {item.details ? (
                  <div className="mt-0.5 line-clamp-2 text-xs leading-5 text-slate-600">{item.details}</div>
                ) : null}
              </div>
            ))}
          </div>
          {pulseWorkToPerformModel.remainingCount > 0 ? (
            <div className="text-xs font-semibold text-slate-500">+{pulseWorkToPerformModel.remainingCount} more</div>
          ) : null}
        </div>
      ) : (
        <p className="mt-2 line-clamp-4 whitespace-pre-wrap break-words text-xs leading-5 text-slate-600">
          {pulseWorkToPerformModel.mode === "summary" ? pulseWorkToPerformModel.body : "No work scope recorded."}
        </p>
      )}
    </div>
  );
