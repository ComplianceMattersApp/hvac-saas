...
import {
  adoptQboPaymentFromForm,
  repairQboPaymentAllocationFromForm,
  syncAttentionPaymentToQboFromForm,
...
export default async function AttentionCenterPage({
  ...
  const canRepairFinancialIdentity = internalUser.role === "admin";
  const model = await buildAttentionCenterReadModel({
    admin: supabase,
  ...
  return <div className={reportPageClass}>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {[
        ...
        { label: "System exceptions", value: model.summaries.systemExceptions },
        { label: "Failed attempts", value: model.summaries.failedPaymentAttempts },
        { label: "Awaiting confirmation", value: model.summaries.fieldPaymentsAwaitingConfirmation },
      ].map(card => <div key={card.label} className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs font-semibold uppercase text-slate-500">{card.label}</div><div className="mt-2 text-3xl font-semibold text-slate-950">{card.value}</div></div>)}
    </div>
    ...
    <section className="space-y-3">
      {model.items.length ? model.items.map(item => <article key={item.id} className={`rounded-xl border p-4 ${item.severity === "critical" ? "border-rose-200 bg-rose-50/60" : "border-amber-200 bg-amber-50/60"}`}>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {item.retryQboPaymentId ? <form action={syncAttentionPaymentToQboFromForm}><input type="hidden" name="payment_id" value={item.retryQboPaymentId} /><SubmitButton loadingText="Retrying..." className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Retry from hub</SubmitButton></form> : null}
          {canRepairFinancialIdentity && item.repairFindingId ? <form action={linkVerifiedStripePaymentIdentityFromForm}><input type="hidden" name="finding_id" value={item.repairFindingId} /><SubmitButton loadingText="Verifying..." className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Link verified Stripe payment</SubmitButton></form> : null}
          {canRepairFinancialIdentity && item.adoptQboFindingId ? <form action={adoptQboPaymentFromForm}><input type="hidden" name="finding_id" value={item.adoptQboFindingId} /><SubmitButton loadingText="Verifying..." className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Adopt QuickBooks payment</SubmitButton></form> : null}
          {canRepairFinancialIdentity && item.repairQboAllocationFindingId ? <form action={repairQboPaymentAllocationFromForm}><input type="hidden" name="finding_id" value={item.repairQboAllocationFindingId} /><SubmitButton loadingText="Verifying and repairing..." className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white">Repair QuickBooks allocation</SubmitButton></form> : null}
          <Link href={item.href} className="inline-flex rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800">{item.actionLabel}</Link>
        </div>
      </article>) : <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-950"><div className="flex items-center gap-2 font-semibold"><CheckCircle2 className="h-5 w-5" />No system exceptions need attention.</div></div>}
    </section>
